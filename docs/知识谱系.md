# 知识谱系

## 宏观全局视图

以复旦微电子学院培养方案为核心坐标，覆盖 IC 工程师所需的完整知识体系。箭头表示"前置依赖"。

```mermaid
graph TB
    %% ======= 基础层 =======
    subgraph F["🏛️ 基础层"]
        direction LR
        MA["数学分析\n微积分·级数·ODE"]
        LA["线性代数"]
        EM["工程数学\n复变函数·积分变换·概率统计"]
        QM["量子力学"]
        SSP["固体物理"]
        SCP["半导体物理"]
        PROG["程序设计基础\nC / Python"]
        DS["数据结构\n与算法"]
    end

    %% ======= 器件与工艺层 =======
    subgraph DP["🔬 器件与工艺"]
        direction TB
        DEV["半导体器件原理\npn结·MOSFET·BJT"]
        PROC["IC工艺原理\n光刻·沉积·蚀刻·掺杂"]
        ADPROC["先进工艺技术\nFinFET·EUV·GAA"]
    end

    %% ======= 数字电路层 =======
    subgraph DIG["🔢 数字电路"]
        direction TB
        DE["数字逻辑基础\n组合·时序·状态机"]
        HDL["硬件描述语言\nVerilog / Chisel"]
        DEDA["数字EDA工具\nVivado / Synopsys DC"]
        FPGA["FPGA 系统设计\n时序约束·IP核·SoC"]
        DIGIC["数字集成电路\n设计原理\nCMOS逻辑·时序·功耗"]
    end

    %% ======= 模拟电路层 =======
    subgraph ANA["〰️ 模拟电路"]
        direction TB
        CB["电路分析基础\nKCL·KVL·阻抗"]
        AE["模拟电子线路\nBJT·MOSFET放大器"]
        AEDA["模拟EDA工具\nCadence Virtuoso\nSpectre 仿真"]
        ANIC["模拟集成电路\n设计原理\n差分对·运放·稳定性"]
        HF["高频电子线路\nS参数·传输线·匹配网络"]
        RF["射频集成电路\nLNA·混频器·VCO·PA·PLL"]
    end

    %% ======= 信号处理层 =======
    subgraph SIG["📡 信号处理"]
        direction TB
        SS["信号与系统\n傅里叶·拉普拉斯·Z变换"]
        MATLAB["MATLAB\n数值计算·仿真"]
        DSP["数字信号处理\nDFT·FFT·FIR·IIR"]
        ADDA["ADC / DAC\n数模转换器设计"]
    end

    %% ======= 系统层 =======
    subgraph SYS["🖥️ 系统架构"]
        direction TB
        ARCH["计算机组成原理\nISA·流水线·Cache"]
        OS["操作系统\n进程·内存管理·驱动"]
    end

    %% ======= 顶层应用 =======
    subgraph TOP["🚀 系统集成与应用"]
        direction LR
        ASIC["ASIC 设计\nRTL→综合→P&R→流片"]
        SOC["嵌入式 SoC\nCPU+外设+软硬协同"]
        EDADEV["EDA 工具开发\n综合算法·布局布线"]
    end

    %% ======= 依赖关系 =======

    %% 数学链
    MA --> EM
    LA --> EM
    MA --> QM

    %% 物理链
    QM --> SSP --> SCP --> DEV
    DEV --> PROC --> ADPROC

    %% 数字链
    SCP --> DE
    DE --> HDL
    HDL --> DEDA
    DE --> DIGIC
    HDL --> DIGIC
    DEDA --> FPGA
    HDL --> FPGA
    DIGIC --> FPGA

    %% 模拟链
    SCP --> CB
    CB --> AE --> ANIC
    AE --> AEDA
    AEDA --> ANIC
    ANIC --> HF --> RF

    %% 信号链
    EM --> SS
    SS --> DSP
    MATLAB --> DSP
    DSP --> ADDA
    DIGIC -.->|混合信号| ADDA
    ANIC  -.->|混合信号| ADDA

    %% 编程链
    PROG --> DS
    DS --> ARCH
    ARCH --> OS

    %% 顶层汇聚
    DIGIC --> ASIC
    ANIC  --> ASIC
    ASIC  --> SOC
    ARCH  --> SOC
    OS    -.-> SOC

    DS    --> EDADEV
    DIGIC --> EDADEV
    ARCH  -.-> EDADEV

    %% 颜色分类
    classDef base    fill:#F5F5F5,stroke:#888,stroke-width:1.5px
    classDef device  fill:#FDE8D8,stroke:#C0530A,stroke-width:2px
    classDef digital fill:#FFFBEB,stroke:#B7791F,stroke-width:2px
    classDef analog  fill:#E6FFEC,stroke:#276749,stroke-width:2px
    classDef signal  fill:#EBF4FF,stroke:#2C5282,stroke-width:2px
    classDef system  fill:#F3E8FF,stroke:#553C9A,stroke-width:2px
    classDef top     fill:#E6F4FF,stroke:#1A365D,stroke-width:2.5px

    class MA,LA,EM,QM,SSP,SCP,PROG,DS base
    class DEV,PROC,ADPROC device
    class DE,HDL,DEDA,FPGA,DIGIC digital
    class CB,AE,AEDA,ANIC,HF,RF analog
    class SS,MATLAB,DSP,ADDA signal
    class ARCH,OS system
    class ASIC,SOC,EDADEV top
```

---

## 分领域详图

### 器件与工艺知识链

```mermaid
graph TB
    QM["量子力学\n波函数·薛定谔方程·量子隧穿"]
    SSP["固体物理\n晶体结构·能带·声子·费米面"]
    SCP["半导体物理\n载流子统计·漂移扩散·pn结I-V"]
    DEV["半导体器件原理\nMOSFET·BJT·短沟道效应"]
    PROC["IC工艺原理\n光刻·氧化·离子注入·CVD/PVD·CMP"]
    ADPROC["先进工艺技术\nFinFET·GAA·HKMG·EUV·SAQP"]
    PDK["工艺设计套件 PDK\n设计规则·标准单元库·SPICE模型"]
    
    QM --> SSP --> SCP --> DEV --> PROC --> ADPROC
    PROC --> PDK
    ADPROC --> PDK
    PDK -.->|供给| 电路设计
    
    classDef device fill:#FDE8D8,stroke:#C0530A,stroke-width:2px
    class QM,SSP,SCP,DEV,PROC,ADPROC,PDK device
```

---

### 数字 IC 设计知识链

```mermaid
graph TB
    DE["数字逻辑基础\n布尔代数·组合逻辑·时序逻辑·FSM"]
    HDLv["Verilog HDL\n模块·always·时序/组合建模"]
    HDLc["Chisel / HLS\n参数化设计·高层次综合"]
    DEDA["数字EDA工具\nVivado / Quartus / Synopsys DC"]
    FPGA["FPGA 设计\n时序约束·IP核·AXI总线·SoC"]
    DIGIC["数字集成电路设计原理\nCMOS逻辑家族·时序分析·功耗·存储器·数据通路"]
    DFT["可测性设计 DFT\n扫描链·BIST·JTAG"]
    ASICFE["ASIC 前端\nRTL编码·功能仿真·逻辑综合·STA"]
    ASICBE["ASIC 后端\n布局规划·CTS·布线·DRC/LVS·Tape-out"]
    UVM["芯片验证\nSystemVerilog·UVM·形式验证"]
    
    DE --> HDLv
    DE --> HDLc
    HDLv --> DEDA
    HDLv --> DIGIC
    DEDA --> FPGA
    HDLv --> FPGA
    DIGIC --> FPGA
    DIGIC --> ASICFE
    HDLv --> ASICFE
    HDLv --> UVM
    ASICFE --> DFT
    ASICFE --> ASICBE
    DFT --> ASICBE

    classDef digital fill:#FFFBEB,stroke:#B7791F,stroke-width:2px
    class DE,HDLv,HDLc,DEDA,FPGA,DIGIC,DFT,ASICFE,ASICBE,UVM digital
```

---

### 模拟 IC 设计知识链

```mermaid
graph TB
    CB["电路分析基础\nKCL·KVL·阻抗·频域分析"]
    AE["模拟电子线路\nBJT·MOSFET·基本放大器·偏置"]
    AEDA["模拟EDA\nCadence Virtuoso + Spectre\n原理图→仿真→版图→DRC/LVS"]
    ANIC["模拟集成电路设计\n差分对·电流镜·运放·稳定性·带宽"]
    BGR["基准与偏置电路\n带隙基准·电流源"]
    HF["高频电子线路\nS参数·传输线·噪声·匹配网络·史密斯圆图"]
    RF["射频集成电路\nLNA·混频器·VCO·PA·PLL·收发机架构"]
    ADDA["ADC / DAC\nSAR·Flash·Pipeline·Sigma-Delta·精度/速度/功耗权衡"]
    
    CB --> AE --> ANIC
    AE --> AEDA
    AEDA --> ANIC
    ANIC --> BGR
    ANIC --> HF --> RF
    ANIC --> ADDA
    
    classDef analog fill:#E6FFEC,stroke:#276749,stroke-width:2px
    class CB,AE,AEDA,ANIC,BGR,HF,RF,ADDA analog
```

---

### 信号处理知识链

```mermaid
graph TB
    EM["工程数学\n复变函数·积分变换·概率论"]
    SS["信号与系统\n连续/离散系统·傅里叶·拉普拉斯·Z变换"]
    MATLAB["MATLAB\n数值计算·Simulink建模·Signal Processing Toolbox"]
    DSP["数字信号处理\nDFT/FFT·FIR/IIR滤波器设计·多速率处理"]
    ADDA["ADC / DAC\n奈奎斯特定理·过采样·量化噪声·Sigma-Delta"]
    IT["信息论\n熵·信道容量·编码定理"]
    COM["数字通信\n调制·编码·均衡·OFDM"]
    
    EM --> SS
    SS --> DSP
    MATLAB --> DSP
    DSP --> ADDA
    IT -.->|可选深化| DSP
    DSP --> COM
    ADDA --> COM
    
    classDef signal fill:#EBF4FF,stroke:#2C5282,stroke-width:2px
    class EM,SS,MATLAB,DSP,ADDA,IT,COM signal
```

---

### 系统架构与 SoC 知识链

```mermaid
graph TB
    PROG["程序设计基础\nC/C++ · Python"]
    DS["数据结构与算法\n树·图·排序·动态规划"]
    ARCH["计算机组成原理\nISA·ALU·流水线·Cache·虚存"]
    OS["操作系统\n进程调度·内存管理·驱动·RTOS"]
    EMB["嵌入式开发\nARM Cortex-M·寄存器编程·外设驱动"]
    SOC["嵌入式 SoC 设计\nCPU核·总线·DMA·IP集成·软硬协同"]
    RISC["RISC-V 处理器实现\n五级流水线·分支预测·Cache设计"]
    
    PROG --> DS --> ARCH --> OS
    ARCH --> EMB --> SOC
    ARCH --> RISC
    OS -.->|软件栈| SOC
    RISC -.->|硬件实现| SOC
    
    classDef system fill:#F3E8FF,stroke:#553C9A,stroke-width:2px
    class PROG,DS,ARCH,OS,EMB,SOC,RISC system
```

---

## 芯片完整生产流程

```mermaid
graph TD
    subgraph FE["🖊️ 前端设计（Frontend）"]
        direction TB
        SPEC["系统规格 / 架构设计"] --> RTL["RTL 编码\nVerilog / SystemVerilog"]
        RTL --> SIM["功能仿真\nVCS / ModelSim"]
        RTL --> SYN["逻辑综合\nSynopsys DC\n→ 门级网表"]
        SYN --> STA1["静态时序分析\nPrimeTime STA"]
        SYN --> DFT["可测性设计 DFT\n扫描链·BIST"]
        SIM --> UVM["形式验证 / UVM\nCadence JasperGold"]
    end
    
    subgraph BE["🔧 后端设计（Backend）"]
        direction TB
        FP["布局规划\nFloorplan + 电源网络"] --> CTS["时钟树综合 CTS"]
        CTS --> PR["布局布线 P&R\nCadence Innovus / ICC2"]
        PR --> STA2["后端 STA\n含寄生参数 RC 提取"]
        PR --> PV["物理验证\nDRC + LVS\nCalibre / PVS"]
        PV --> GDS["GDSII 交付\nTape-out"]
    end
    
    subgraph FAB["🏭 制造与测试"]
        direction TB
        MASK["掩膜版制作"] --> WAFER["晶圆制造 Fab\n(光刻→蚀刻→沉积循环)"]
        WAFER --> CP["晶圆级测试 CP"]
        CP --> PKG["切割与封装"]
        PKG --> FT["成品测试 FT\nATE 测试机"]
        FT --> SHIP["量产出货"]
    end
    
    FE --> BE
    GDS --> MASK
    DFT -.->|测试向量| FT
    
    classDef fe fill:#FFFBEB,stroke:#B7791F,stroke-width:2px
    classDef be fill:#E6FFEC,stroke:#276749,stroke-width:2px
    classDef fab fill:#EBF4FF,stroke:#2C5282,stroke-width:2px
    class SPEC,RTL,SIM,SYN,STA1,DFT,UVM fe
    class FP,CTS,PR,STA2,PV,GDS be
    class MASK,WAFER,CP,PKG,FT,SHIP fab
```

---

## 学习路径建议

### 数字方向（Digital IC / ASIC 工程师）

```
数学分析 + 线性代数
    → 量子力学 → 固体物理 → 半导体物理
    → 半导体器件原理
    → 数字逻辑基础
    → Verilog HDL + 数字EDA工具（Vivado）
    → 数字集成电路设计原理
    → FPGA 系统设计（验证平台）
    → 芯片验证（SystemVerilog / UVM）
    → ASIC 前端（综合/STA）
    → ASIC 后端（P&R/物理验证）
    → 流片
```

### 模拟方向（Analog IC / RF 工程师）

```
数学分析 + 工程数学（复变/积分变换）
    → 量子力学 → 固体物理 → 半导体物理
    → 半导体器件原理
    → 电路分析基础 → 模拟电子线路
    → 模拟EDA（Cadence Virtuoso）
    → 模拟集成电路设计原理
    → 高频电子线路
    → 射频集成电路（LNA/VCO/PLL）
```

### 信号处理 / 混合信号方向

```
数学分析 + 线性代数 + 工程数学
    → 信号与系统 + MATLAB
    → 数字信号处理（DSP）
    → 模拟IC基础（模拟电子线路）
    → ADC / DAC 设计
    → 混合信号系统
```

### SoC / 嵌入式方向

```
程序设计基础（C/C++）
    → 数据结构与算法
    → 计算机组成原理
    → 操作系统 / RTOS
    → Verilog + FPGA（硬件部分）
    → 嵌入式 SoC 设计（软硬协同）
```
