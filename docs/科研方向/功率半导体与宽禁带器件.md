...
这些难处没拦住人们往更高处看。既然把禁带垫高这么管用，能不能再宽一点？沿着这个念头往前，就是氧化镓和金刚石这类超宽禁带材料，禁带宽到 4.8 甚至 5.5 电子伏特，扛电压的本事比碳化硅、氮化镓还要高一截。氧化镓尤其诱人，因为它能像拉硅锭那样从熔体里直接长出大块单晶，成本和尺寸上比靠高温高压才长得出来的碳化硅、氮化镓都占便宜。可它的软肋也更极端，几乎不导热，热量全憋在器件里出不来，而且至今做不出像样的 p 型。于是散热这件事，从氮化镓一路追到超宽禁带，越往前越成了决定材料能不能落地的命门。
...
课程信号分两侧。器件侧看固体物理和半导体物理，能不能从禁带宽度一路推到耐压和损耗，决定你读不读得懂 SiC 和 GaN 好在哪。电路侧看模拟电路和电磁场，门极驱动、寄生参数这些活全压在这两门课上。日常也分两种过法。器件侧的人在 TCAD 仿真和高压测试台之间来回，搭双脉冲测试平台把仿真和实测对上，再做温循、老化这类可靠性试验，一颗器件从设计到敢说可靠常以季度甚至年计。电路侧的人盯着纳秒级的开关瞬间，几纳亨寄生电感就能激起上百伏过冲，版图和封装要贴着器件物理一轮轮改。性格上适合两类人。一类看到 I-V 曲线异常会兴奋，想往下挖是哪层物理出了问题；一类喜欢实在感，自己的东西最终要在几百伏、几百安下连跑十年不出岔子，这种约束让人踏实而不是烦躁。提醒两点。实验室里高压大电流是真危险，操作规程一条都不能省。节奏也确实慢，舞台是 IEDM 和 ISPSD，想要短平快反馈、或者只想写代码不想碰功率台架的人会熬不住。
