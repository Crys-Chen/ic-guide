...
这个方向适合享受被规格逼到墙角的人，这里每个晶体管尺寸的取舍背后都有真实的物理意义，对大脑怎么编码意图也带着好奇心。微电子本科最自然的切入口就是它的硬件这一侧。低噪声放大器、ADC、片上 spike 检测、安全的刺激器前端全是模拟与混合信号 IC 的硬核活，把 NEF 逼向物理极限正是脑虎、Neuralink 这类侵入式 BCI 团队最缺的本事，成果常发在 ISSCC。诚实提醒一句，只懂电路远远不够，你还得忍受大脑漂移、动物到人隔着鸿沟、一次反馈等几个月的慢节奏。
