...
这个方向天生跨层，课程信号也是组合拳，半导体器件、模拟电路、计算机组成至少两门有感觉才玩得转，其中模拟电路是最硬的一块，读出和 ADC 是绕不开的活。日常看你落在哪一层。器件组在测试台上对着 RRAM 阵列量变异和漂移；电路组在 Cadence 里设计 CIM 宏单元、等流片、回片测试，成果挂在 ISSCC；架构组写仿真器、跑量化实验，节奏接近系统方向。但不管从哪层进来，迟早要把另外两层补个大概，因为这个方向的核心矛盾全长在层与层的接缝上，ADC 吃掉阵列省下的能量，量化精度顶着器件的有效位，单层视角看不见全局，所以它适合喜欢来回穿梭甚于在一层里扎到底的人。性格上还要能忍受不干净。看到模拟有效位只有 4-6 位、温漂让昨天的校准今天失效这类又脏又具体的矛盾，觉得有意思而不是烦躁，就是对的信号；只想做干净理论推导的人会很难受。提醒一句，这个领域热度高、论文多，反而更考验判断力，模拟路线的精度之争悬了十年没有定论，选题时要分得清哪些是真矛盾、哪些是为发论文造出来的赛道。
