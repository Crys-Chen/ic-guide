...
这个方向天生跨层，适合愿意在器件、电路、架构之间来回穿梭的人。看到模拟有效位只有 4-6 位、ADC 开销抵消阵列收益这类又脏又具体的矛盾会觉得有意思，那就对了。微电子本科最稳的切入口在器件和模拟电路这一侧，从 RRAM 阵列、SRAM-CIM 单元或读出 ADC 入手，设计阵列、分析噪声、做流片，成果发在 ISSCC，这正是知存、后摩这类存算芯片公司最缺的人。诚实提醒一句，它的价值恰恰长在不干净的物理细节里，只想做干净理论推导的人会很难受。
