...
- **模拟与数字路线之争**：模拟 CIM 用电流和基尔霍夫定律换来 100× 能效，代价是有效位常只有 4-6 位，数字 SRAM-CIM 精度可控、能复用成熟 EDA、量产风险低但能效只有 3-5×，孰优孰劣尚无定论。
...
这个方向天生跨层，适合愿意在器件、电路、架构之间来回穿梭的人。看到模拟有效位只有 4-6 位、ADC 开销抵消阵列收益这类又脏又具体的矛盾会觉得有意思，那就对了。微电子本科最稳的切入口在模拟电路和器件这一侧，从 SRAM-CIM 或 RRAM 器件入手，设计阵列、分析噪声、做流片，成果发在 ISSCC 这类场子。诚实提醒一句，它的价值恰恰长在不干净的物理细节里，只想做干净理论推导的人会很难受。
