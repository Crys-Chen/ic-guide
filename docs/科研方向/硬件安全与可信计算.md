...
- **硬件漏洞无法升级**：Spectre 这类漏洞来自三十年前"天经地义"的投机执行优化，刻在硅上无从回收，"修复"只能靠禁用功能换 5%–30% 性能。当性能优化与安全天然对立，微架构该如何在设计期就把侧信道堵死，而不是事后打补丁？
- **供应链信任与硬件木马检测**：一颗芯片要过设计、IP、代工、封装数家之手，任何一环都可能植入只在特定条件触发的隐藏电路。几十亿晶体管里揪出极小一撮木马，功能仿真测不到、电镜逐层扫描又贵到只有国安场景用得起——侧信道指纹、ML 版图异常检测、RTL 形式化验证，谁能逼近"完全可靠"？
- **物理泄漏的猫鼠游戏**：芯片做加密时，功耗、电磁、时序都在持续出卖密钥（SPA/DPA/EMA），如今连 NPU 的权重都能从功耗曲线反推。掩码（masking）、恒时执行（constant-time）这些物理层消噪手段，永远在被新的测量方法追赶——这场博弈关得上吗？
- **PUF 的稳定与抗建模**：把制造偏差变成硅片"指纹"，难点在两端——如何让响应在温度漂移与器件老化下仍稳定重现，又如何抵抗攻击者用大量激励-响应对训练出的机器学习数学复制品？
- **可信执行环境的边界**：ARM TrustZone、RISC-V PMP 想在硬件里划出"安全飞地"，让操作系统被攻陷时飞地内代码仍不被窥探。但 TEE 自身也不断被微架构侧信道击穿，这块"信任根"的边界究竟能守多牢？
- **让硬件不必被信任**：从姚期智的"百万富翁问题"到安全多方计算，FHE 让密文上直接算、ZKP 证明为真而不泄露细节——硬件由此从"信任之根"变成密码协处理器。可 NTT、大整数模乘、MSM 让纯软件慢上万倍，必须靠专用加速器（FHE/ZK accelerator）救场，算法与硅如何协同把它拉回实用？

### 知识路径

```mermaid
graph LR
    A["数字电路 / HDL\n功耗·时序的物理来源"] --> B["ASIC 设计流程\n木马·工艺偏差在哪产生"]
    B --> C["体系结构 / 微架构\n投机执行·侧信道·TEE"]
    C --> D["系统安全 / 操作系统\nTrustZone·PMP·内核权限"]
    D --> E["硬件安全与可信计算\n攻防博弈"]

    F["密码学\nAES·RSA · MPC/FHE/ZKP"] --> E
    F --> G["密码加速器\nNTT·MSM·盲算硅"]
    G --> E
...
- 数字电路 / HDL → [电路（数字）](../学习地图/电路/数字/index.md)、[电路总览](../学习地图/电路/index.md)
- ASIC 设计流程 → [电路 · ASIC 设计](../学习地图/电路/ASIC/index.md)
- 体系结构 / 微架构 → [系统架构 · 体系结构](../学习地图/系统架构/体系结构/index.md)
- 系统安全 / 操作系统 → [系统架构 · 系统安全](../学习地图/系统架构/系统安全/index.md)、[系统架构 · 操作系统](../学习地图/系统架构/操作系统/index.md)
- 密码学（含 MPC/FHE/ZKP 与加速器的数学底座）→ [数学进阶 · 密码学（Stanford）](../学习地图/数学/数学进阶/密码学/StandfordCrypto.md)、[数学进阶](../学习地图/数学/数学进阶/index.md)

## 这个方向适合谁

这个方向天生吸引一种人：看到任何系统，第一反应不是"它怎么工作"，而是"它会从哪里被攻破"。你得同时扮演攻击者和防御者——先以攻击者的偏执去想象那些非常规的泄漏路径（功耗拍子、缓存余温、一段沉睡的木马电路、温度漂移下走形的 PUF 响应），再切回防御者把它们一条条堵死。威胁建模是这里的母语：先把"敌人能做什么、看得见什么"讲清楚，研究才有立足点。如果你享受这种攻防博弈、对"硅片上的秘密如何泄露出来"有近乎执拗的好奇，这个方向会让你上瘾。

入口因背景而分岔，但都通向同一片战场。EE 背景的人从物理层进来最自然：你懂功耗如何随数据起伏、时序路径怎么形成、制造偏差从何而来——这正是侧信道、PUF、硬件木马的物理根基，顺着数字电路与 ASIC 版图往上走即可。CS 背景的人从系统层切入：操作系统、TEE 机制、编译器如何生成恒时代码，都是合理起点；Spectre 这类漏洞本质是微架构与软件抽象层之间的错位，恰好要求你两头都懂。而正文里那条"让密码学接管计算"的激进战线（FHE、ZKP、安全多方计算与它们的专用加速器）则更吃数学——如果你既迷恋密码协议、又想把它在硅上跑快上万倍，这里有一块少有人同时占据的交叉地带。

不必有顶级数学功底才能入门前三条战线：把 AES、RSA 的基本结构吃透就够开工，不必从头推导安全证明（但若想做 FHE/ZK 加速，数论与代数的分量会明显加重）。真正的门槛是对细节的耐心——功耗曲线分析、时序路径排查、版图比对，都是需要反复盯着数据看的慢功夫，急性子在这里会很痛苦。

发表阵地的现实是：这是一个真正横跨两个社区的方向。硬件侧有 HOST、CHES（论文进 IACR TCHES）、DAC，安全侧有 IEEE S&P、ACM CCS、USENIX Security、NDSS，密码加速与协议还会触到密码学本身的圈子。一篇好工作往往能同时被硬件人和安全人引用，读博期间就能在两边都攒下声誉——这种跨界身份是稀缺的。代价是节奏：顶会评审挑剔、攻防你来我往，一个防御方案可能很快被新攻击打穿，得有"长期作战"的心理准备。如果你还在意供应链安全、可信计算这类有明确政策意义与产业需求的命题，这个方向给你的不只是论文，还有实打实的落地舞台。
...
-   **[纪志罡](https://icisee.sjtu.edu.cn/jiaoshiml/jizhigang.html)** <span class="badge-other">交大</span>
    ...
    微纳电子器件硬件安全 · 真随机数/PUF 物理原语 · 器件-电路协同可靠性

-   **[胡云聪（Yuncong Hu）](https://huyuncong.com/)** <span class="badge-other">交大</span>
...
<div class="dm-wrap chip-block">
  <div class="dm-row dm-device"><span class="dm-lbl">会议</span><span class="dm-chips">
    <span class="dm-chip dm-chip--static" title="硬件安全专属">HOST</span>
    <span class="dm-chip dm-chip--static" title="IEEE 安全与隐私研讨会（Oakland）">IEEE S&P</span>
    <span class="dm-chip dm-chip--static" title="ACM 计算机与通信安全会议">ACM CCS</span>
    <span class="dm-chip dm-chip--static" title="USENIX 安全研讨会">USENIX Security</span>
    <span class="dm-chip dm-chip--static" title="网络与分布式系统安全研讨会">NDSS</span>
    <span class="dm-chip dm-chip--static" title="设计自动化大会">DAC</span>
    <span class="dm-chip dm-chip--static" title="密码硬件与嵌入式">CHES</span>
  </span></div>
  <div class="dm-row dm-arch"><span class="dm-lbl">期刊</span><span class="dm-chips">
    <span class="dm-chip dm-chip--static" title="信息取证与安全">IEEE TIFS</span>
    <span class="dm-chip dm-chip--static" title="可信与安全计算">IEEE TDSC</span>
    <span class="dm-chip dm-chip--static" title="IACR 密码硬件与嵌入式系统汇刊">IACR TCHES</span>
    <span class="dm-chip dm-chip--static" title="IEEE 超大规模集成系统汇刊">IEEE TVLSI</span>
    <span class="dm-chip dm-chip--static" title="ACM 电子系统设计自动化汇刊">ACM TODAES</span>
  </span></div>
</div>
...
<div class="dm-wrap chip-block">
  <div class="dm-row dm-device"><span class="dm-lbl">国内</span><span class="dm-chips">
    <a class="dm-chip" href="http://www.iie.ac.cn/" title="网络空间安全防御全国重点实验室、处理器安全架构与国产 CPU 安全">中科院信息工程研究所（信工所）</a>
    <a class="dm-chip" href="https://www.sic.tsinghua.edu.cn/" title="可重构密码芯片、PUF/TRNG、后量子密码芯片">清华大学集成电路学院</a>
    <a class="dm-chip" href="https://loccs.sjtu.edu.cn/" title="密码工程、侧信道分析、密态计算">上海交大密码与计算机安全实验室（LoCCS）</a>
  </span></div>
  <div class="dm-row dm-arch"><span class="dm-lbl">国外</span><span class="dm-chips">
    <a class="dm-chip" href="https://www.csail.mit.edu/" title="硅基 PUF、AEGIS 安全处理器（Devadas 组）">MIT CSAIL</a>
    <a class="dm-chip" href="https://fics.institute.ufl.edu/" title="硬件木马、IC 供应链安全">佛罗里达大学 FICS Research</a>
    <a class="dm-chip" href="https://www.mpi-sp.org/" title="嵌入式安全、硬件逆向工程">马普安全与隐私研究所（MPI-SP）</a>
    <a class="dm-chip" href="https://www.iaik.tugraz.at/" title="微架构侧信道（Meltdown/Spectre 发现地之一）">TU Graz IAIK</a>
  </span></div>
</div>
