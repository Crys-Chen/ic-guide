...
- **硬件漏洞无法升级**：Spectre 来自投机执行这种天经地义的优化，刻在硅上无从回收，修复只能靠禁用功能换 5%–30% 性能，逼着微架构在设计期就把侧信道堵死。
- **供应链信任与硬件木马**：芯片要过设计、IP、代工、封装数家之手，几十亿晶体管里揪出极小一撮只在特定条件触发的隐藏电路，功能仿真测不到、电镜逐层扫描又贵到只有国安场景用得起。
- **物理泄漏的猫鼠游戏**：加密时功耗、电磁、时序都在出卖密钥（SPA/DPA/EMA），如今连 NPU 权重都能从功耗曲线反推，掩码和恒时执行这些消噪手段永远在被新测量方法追赶。
- **PUF 的稳定与抗建模**：把制造偏差变成硅片指纹，难在让响应在温漂和老化下仍稳定重现，又要抵抗攻击者用大量激励-响应对训练出的数学复制品。
- **可信执行环境的边界**：TrustZone、RISC-V PMP 想划出操作系统被攻陷时仍不被窥探的安全飞地，但 TEE 自身也不断被微架构侧信道击穿。
- **让硬件不必被信任（IC 切口）**：FHE 在密文上直接算、ZKP 证明为真而不泄露细节，可 NTT、大整数模乘、MSM 让纯软件慢上万倍，必须靠专用加速器把它拉回实用。
...
这个方向吸引看到任何系统先想它会从哪里被攻破的人，要同时扮演攻击者和防御者，享受攻防博弈、对硅片上的秘密如何泄露有执拗的好奇就对了。微电子本科从物理层进来最自然。你懂功耗如何随数据起伏、时序路径怎么形成、制造偏差从何而来，这正是侧信道、PUF、硬件木马的物理根基，顺着数字电路和 ASIC 版图往上走即可，想再深一步还能去做 FHE 和 ZKP 的专用加速器。诚实提醒一句，前几条战线不必有顶级数学功底，把 AES、RSA 吃透就够开工，但真正的门槛是对细节的耐心。功耗曲线分析、时序排查、版图比对都是反复盯数据的慢功夫，而且一个防御方案可能很快被新攻击打穿，急性子会很痛苦。成果横跨 HOST 和 IEEE S&P 两个社区。
