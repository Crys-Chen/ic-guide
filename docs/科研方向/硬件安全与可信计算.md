...
- **硬件身份与可信边界**：PUF 把制造偏差变成硅片指纹，难在温漂老化下仍稳定重现又要抗建模攻击，往上 TrustZone、RISC-V PMP 想划出的安全飞地也不断被微架构侧信道击穿。
...
这个方向吸引看到任何系统先想它会从哪里被攻破的人，要同时扮演攻击者和防御者，享受攻防博弈就对了。微电子本科从物理层进来最自然，你懂功耗如何随数据起伏、时序路径怎么形成、制造偏差从何而来，这正是侧信道、PUF、硬件木马的物理根基，顺着数字电路和 ASIC 版图往上走，想再深一步还能去做 FHE 和 ZKP 的专用加速器，成果常发在 HOST。诚实提醒一句，功耗曲线分析、版图比对都是反复盯数据的慢功夫，一个防御方案可能很快被新攻击打穿，急性子会很痛苦。
