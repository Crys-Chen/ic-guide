...
FHE 和 ZKP 在硬件上都有一大瓶颈——算力不足。这两种密码学算法都需要 **NTT（数论变换）**、大整数模乘和 **MSM（多标量乘法）** 等复杂算子。纯软件实现比明文慢上万倍，靠数学走不动，必须有专用的硬件加速。于是面向密码学的专用加速器近年来也开始出现。MIT、Intel 都做了 FHE 加速器原型；ZK 方向有 **Cysic**（中国）、**Ingonyama** 这类 2022 年后涌现的初创公司，专门加速 prover。
