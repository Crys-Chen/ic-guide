...
<div><svg viewBox="0 0 860 340" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:860px;display:block;margin:1.5rem auto;">
  ...
  <text x="430" y="30" text-anchor="middle" font-size="13" font-weight="bold" fill="#1E293B">为什么 FPGA 适合边缘实时推理：批处理 vs 数据流流水线</text>
  ...
  <text x="58" y="132" font-size="9.5" fill="#64748B">攒一批再算</text>
  <text x="270" y="90" text-anchor="middle" font-size="9" fill="#64748B">帧2</text>
  <text x="320" y="90" text-anchor="middle" font-size="9" fill="#64748B">帧3</text>
  <text x="370" y="90" text-anchor="middle" font-size="9" fill="#64748B">帧4</text>
  ...
  <text x="335" y="121" text-anchor="middle" font-size="10.5" fill="#475569">等齐一批（帧2/3/4 还在路上）</text>
  ...
  <text x="58" y="232" font-size="9.5" fill="#64748B">定制流水线</text>
  ...
  <text x="430" y="216" font-size="9.5" fill="#64748B">帧1 在层2 时，帧2 已进层1，逐帧流过不必等批</text>
  ...
  <text x="430" y="300" text-anchor="middle" font-size="10.5" fill="#475569">边缘要的是每帧准时出结果。GPU 靠攒批换吞吐，单帧延迟高；FPGA 把网络铺成流水线，</text>
...
- **灵活性与效率的硬矛盾**：FPGA 卡在 CPU 的可编程性和 ASIC 的极致性能之间，可重构带来的路由开销让面积的七八成都耗在连线上，怎么保留可重构又把这部分损失压到最小是根本问题。
- **布局布线（P&R）**：逻辑摆哪个单元、信号走哪条线是 NP 难组合优化，大型设计动辄跑几十小时还受随机种子影响，怎么用新启发式或机器学习同时加速时序收敛、降低方差仍没解。
- **HLS 质量差距**：用 C/C++ 自动生成 RTL 能大幅缩短开发周期，但时钟频率比手写低两到五成、面积更大，循环展开和流水线插入的 pragma 设计空间是指数级的，怎么让工具自动逼近手写质量是热点。
- **神经网络映射（IC 切口）**：把量化到 4 位甚至 2 位的 DNN 各层算子高效铺到 DSP/LUT/BRAM 上、最大化数据局部性、最小化片外访存，是低延迟边缘推理能否打过 GPU 的关键。
- **数据中心运行时可重构**：FPGA 进了机架要面对多租户和动态负载，部分重构怎么在运行时调度多个加速器、把可重构从编译期能力变成真正的运行时灵活性仍是开放问题。
- **该硬化什么、异构怎么切**：哪些功能该硬化成专用电路、硬到什么程度、占多大面积是一场赌博，硬化省了效率却赌应用用得上、用不上就白占地盘，如今 DSP、BRAM、硬核 NoC、AI 引擎、ARM 子系统这套异构资源怎么协同没有定论。
- **粗粒度可重构（CGRA）**：FPGA 的 bit 级灵活到极致、连线代价也大到极致，把颗粒做粗让完整运算单元连成数据通路能让效率向 ASIC 靠拢，但颗粒该粗到哪一档、编译器怎么映射是国内软件定义芯片主攻的难题。
- **可信与可靠**：比特流是芯片的全部知识产权、不加密就能被原样复制，多租户共用一块 FPGA 如何隔离防侧信道、航天场景如何抗辐射加固都还要啃。
...
这个方向最适合写完就想下板的人，比起停在仿真波形里反复看时序图，你更想把比特流灌进真实硬件按真实时钟跑端到端系统。一个架构改动几小时就能在板上看到结果，研究成本远低于动辄数月、数百万元的 ASIC 流片。微电子本科在这里几乎是主场，数字电路和 Verilog 是硬门槛，过了之后路有好几条。偏软的人从 C/C++ 切进 HLS、研究怎么让工具自动逼近手写质量，喜欢算法的人钻布局布线这个工程约束明确的 NP 难问题，想做系统的人把量化后的 DNN 算子高效铺到 DSP/LUT 上，硬件、算法、系统三种口味都容得下。诚实提醒一句，这个圈子有成建制的社区（FPGA、FCCM）和 VTR、AWS F1 这类成熟平台兜底，节奏友好，但 FPGA 比 ASIC 慢四倍、大三十倍的硬伤是它先天的天花板。
