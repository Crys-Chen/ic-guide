...
课程信号很直接，数字逻辑和 Verilog，在 HDLBits 上刷题刷得停不下来就是信号；想往工具侧走还要加数据结构和算法，布局布线本质是 NP 难的组合优化，和刷题用的图论是同一套肌肉。日常围着板子转。写 RTL，等综合和布局布线，大设计一跑几十小时，然后上板，用 ILA 抓波形 debug，时序不收敛就回去读报告改约束。它适合想做硬件但等不起流片的人，一个架构改动几小时就能在真实硬件上看到结果，这种反馈速度在硬件方向里数一数二；门槛也低，一块几百块的开发板就能开工，本科生攒第一段硬件科研经历，从这里起步最容易。性格上适合动手见效型，把完整系统跑通的成就感大于把单点理论推到极致。三种口味都容得下，喜欢搭电路的做加速器，喜欢算法的做 P&R 和 HLS 工具，喜欢系统的玩数据中心和部分重构。提醒一句，FPGA 比 ASIC 慢四倍、大三十倍是先天天花板，这个方向的价值在灵活和低延迟，不在峰值算力，拿它跟 GPU 拼吞吐是走错了考场。
