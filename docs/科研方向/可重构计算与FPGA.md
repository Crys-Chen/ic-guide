...
软件再聪明，也只能在既定的硅片上榨，可那一半以上的连线浪费是 fabric 本身定死的。要再往下压，就只能修改硬件了。FPGA 架构师面临的核心问题是：<u>哪些功能该硬化成专用电路，硬到什么程度，占多少芯片面积？</u> 这是一场赌博：把一个功能焊进硅里，用得上它的应用立刻更小更快更省电；可一旦某块芯片碰上的应用根本用不到它，这块硅就白占了本该留给通用逻辑的地盘。
