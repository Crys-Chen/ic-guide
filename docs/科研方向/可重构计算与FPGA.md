...
这个方向最适合写完就想下板的人，比起停在仿真波形里看时序图，你更想把比特流灌进真实硬件跑端到端系统，一个架构改动几小时就能在板上看到结果。微电子本科在这里几乎是主场，数字电路和 Verilog 过了之后，HLS、布局布线、DNN 算子铺到 DSP/LUT，硬件算法系统三种口味都容得下。诚实提醒一句，这个圈子社区成熟、节奏友好，但 FPGA 比 ASIC 慢四倍、大三十倍的硬伤是先天天花板。
