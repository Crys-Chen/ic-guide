...
汽车以 60 公里时速撞上护栏，车身开始形变的那一刻，气囊必须在 20 毫秒内充气。再晚一点，驾驶员的头就撞上方向盘了。触发这整个过程的，是一块芯片上一根几十微米的硅弹簧。碰撞产生的减速度让弹簧末端的质量块偏移，两侧梳齿间电容变化，信号送出，点火。这根弹簧不是机床加工的，而是用光刻胶和刻蚀液从硅晶圆上直接雕出来的，和做处理器用的是同一套工艺。今天每部 iPhone 里至少有五个这样的器件：加速度计感知方向让画面随握持旋转，陀螺仪让 AR 防抖成为可能，气压传感器辅助室内定位，好几颗 MEMS 麦克风把声波转成电信号、协同降噪。它们物理原理各不相同，却都建立在同一套半导体工艺平台上，单颗几分钱，一片晶圆同时出几千颗。MEMS 的本质只有一件事：用 IC 工艺把某种物理现象翻译成电信号。
...
课程信号有点反常识，这个方向最吃的恰恰是很多微电子学生最先丢掉的内容，大学物理的力学部分，梁的刚度、振动、阻尼这一套，再加上模拟电路的噪声分析。这两块都觉得有意思的人不多，但都觉得有意思就是强信号。日常分三段。设计阶段泡在多物理场仿真里，把质量块面积、弹簧刚度、梳齿间距、气膜阻尼放在一起调，改一个参数动全局，像在解一个永远欠定的方程组。工艺阶段进超净间走光刻、刻蚀、释放，结构是悬空的，粘连和残余应力随时让一批片子报废。测试阶段在探针台和振动台上让器件真的动起来，量出实测曲线才算数。性格上适合喜欢跨界折腾的人，左手力学右手电学，有动手做实物的工程师趣味，还坐得住设计、流片、工艺、测试一圈几个月的长闭环。提醒两句。这一行的硬通货是真器件跑出的曲线，仿真图说服不了 Transducers 的审稿人；习惯了改两行代码就看结果的人会被这个节奏磨疯。反过来，亲手让一个几十微米的机械结构在芯片上动起来、把一个分子的质量从频率偏移里数出来，这种成就感纯仿真和纯软件方向都给不了。
