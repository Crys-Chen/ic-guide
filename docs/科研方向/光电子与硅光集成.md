...
除了 MZI 干涉网格，微环谐振器（Microring Resonator, MRR）阵列是另一条主流的光计算路径。MZI 把权重刻进相移器配置，MRR 则利用谐振耦合实现权重乘加：输入向量各分量由不同波长的光承载（WDM），经过一列微环组成的"权重库"，每个微环通过调节谐振峰的位置控制对应波长光的透过率，输出端的光电探测器直接读出乘加结果。MRR 比 MZI 更紧凑——单个微环只需几十微米，且天然支持波分复用带来的通道并行，但代价是谐振状态对温度漂移和制造偏差极为敏感，相邻微环间的串扰也更难压制。
...
但光计算面前还横着四座大山。第一是非线性。神经网络要靠 ReLU、sigmoid 这类非线性激活函数才能逼近复杂函数，纯线性光学的干涉、分束、相移只能做线性变换。实现非线性的方案目前有几条：最主流的是把光转回电做完非线性再转回光（光-电-光方案），简单直接但每次跨域都要付出电光转换代价；相变材料（Phase-Change Material, PCM）如 GST 薄膜可以利用非易失的光学态切换实现原位非线性，但写入速度和循环寿命仍是瓶颈；电吸收调制器（EAM）利用半导体异质结的吸收边非线性，速度快但调制深度有限；微环自身的双稳态谐振响应也能充当非线性激活函数，但工作点对温漂极为敏感。整体而言，除光-电-光方案外，其余非线性方案目前都停在小规模研究阶段，尚未达到可工程化集成密度。第二是光电转换。输入往往来自电域存储器，输出也要回电域使用，每一层非线性再绕回电域一次，电光转换的固定代价就会吃掉计算上省下的能耗。第三是精度。温度漂移和制造偏差都会引起相位误差，有效精度通常只剩 4 到 6 位，AI 模型即使 INT8 也比这宽。第四是集成度。一个 MZI 单元几十到上百微米，比晶体管大好几个数量级，一个 1024×1024 矩阵就要上百万个 MZI，现有工艺单芯片装不下。

此外，目前主流的光计算范式可以说是摸着"**模拟存算一体**"过河，二者在方法论和所面临的瓶颈上高度相似。两者都把运算交给物理过程自然完成，前者靠光的干涉，后者靠欧姆定律和基尔霍夫定律，也因此共享精度、集成度、噪声漂移这些模拟计算的同源问题。但光计算多背两座山。模拟存算始终待在电域里，非线性用晶体管直接解决，输入输出也都是电信号不用跨域，这两样光计算都躲不开，产业落地也就比模拟存算更晚一步。

面对这些限制，"全光"并非唯一出路，更务实的思路是光-电协同设计（photonic-electronic co-design）。不再追求把每一层都做在光域里，而是按算子特性分工：固定权重的线性变换（如推理时的矩阵乘法）交给光子阵列完成，利用其高带宽低功耗优势；动态算子——注意力权重的实时计算、非线性激活函数、路由控制——交给电子 systolic array 或专用数字逻辑处理。两者之间通过密集的光电转换接口级联，形成一个混合流水线。这种架构的关键在于合理的算子分区（partition）：如果光子段足够长、跨域次数足够少，光电转换的固定代价就能被光段省下的能耗摊薄；反之，频繁跨域则抵消光计算的全部优势。这条路究竟能走多远，业界至今没有定论。
...
-   **[安俊明](https://people.ucas.edu.cn/~anjunming1010)** <span class="badge-other">中科院</span>
...
- **[Deep learning with coherent nanophotonic circuits](https://doi.org/10.1038/nphoton.2017.93)**
    Nature Photonics · 2017 · MIT 开山之作，首次在硅光子芯片上用 56 个 MZI 组成的可编程阵列实现光学神经网络推理

- **[Photonic multiply-accumulate operations for neural networks](https://doi.org/10.1109/JSTQE.2019.2941485)**
    IEEE J. Sel. Top. Quantum Electron. · 2020 · 系统性对比光子与电子 MAC 硬件性能边界，给出光计算在能效、速度、计算密度上的量化优势区间

- **[基于微环谐振器的光子计算](https://doi.org/10.3788/AOS250932)**
    光学学报 · 2025 · 黄超然团队特邀综述，从微环谐振器原理出发，系统梳理基于 MRR 的线性计算、非线性计算及光-电-光方案，并讨论精度、鲁棒性、扩展性等挑战
