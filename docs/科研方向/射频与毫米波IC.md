...
二十年前，一部手机只能打电话、发短信；今天，你在时速三百公里的高铁上还能开视频会议、刷直播。这是无线的一副面孔，越来越快。它还有另一副，方向几乎相反。你手腕上的手环、口袋里的门禁卡、甚至植入体内的起搏器，靠一颗小得可怜的电池就能安静工作十年，这是它越来越省、越来越无处不在的一面。同一门射频学问，被这两股需求往相反的方向拉扯。它们争的其实是同一件事的两本账：信号从发射机走到接收机，这一路的得与失都要算清。一本账叫链路预算，盯着信号还传不传得到；一本账叫能量预算，盯着这一下花得起多少电。这个方向几乎所有的研究，都是在替这两本账跟物理讨价还价。

无论你想把信号往更高的频率上推，还是想让它省到极致，都得先迈过同一道坎，而这道坎就藏在电路内部。在低频世界里，工程师可以把一根导线看成理想的连接：电流从这头到那头，不损耗、不延迟、不辐射。可只要信号频率爬进 GHz 量级，这个假设就彻底失效。一根几毫米长的走线，其电感就足以改变信号的相位；两根挨着的走线，寄生的耦合电容能把一路信号悄悄漏给另一路；晶体管自身的增益还会随频率一路下滑。射频工程师不得不换一套语言来描述电路，用 S 参数代替增益，用噪声系数衡量灵敏度，用史密斯圆图判断阻抗匹配得好不好。这套语言是模拟电路在高频下的延伸，物理图像却已面目全非。迈过这道共同的坎，才谈得上往各自的方向使劲。
