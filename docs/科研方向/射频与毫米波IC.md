...
移动通信从 2G、3G、4G 一路走到 5G，短短二十来年，手机从只能打电话、发短信，变成能在时速三百公里的高铁上看直播、开视频会议，每往前一代，能传的数据都多出一大截。换个场景，手腕上的运动手环、植入体内的起搏器，靠一颗小电池就能安静工作十年。这两类无线需求几乎相反，归根到底却是同一件事：多快好省地传输信息。然而一颗芯片不可能既要又要。要又多又快，就得占用更宽的频段、推向更高的频率，可频率越高越费电、越脆弱；要传得准，就得多花功率和带宽。于是这门学问分成两路，一路往高频走、争带宽，一路往低功耗压、保续航。
...
发射端的 PA，是两种需求第一次正面相遇的地方。要传得远，PA 就得输出足够大的功率，可大功率会让晶体管进入非线性区，产生谐波、干扰相邻信道；要保持线性，又得降低工作点，效率随之下降。一个 4G 基站的 PA，效率常年只有三四成，其余的电都变成了热。这是大功率高速电路绕不开的烦恼。而靠一颗纽扣电池过活的物联网芯片，难处恰好相反：电池要撑十年，PA 就不能一直开着，于是它大部分时间休眠，只在该发送时醒来，发完立即休眠，每比特的能耗都要尽量压低。同一个放大器，做基站的一心提高功率和效率，做传感器的一心压低功耗。

信号一旦离开天线，损耗就开始了。电磁波在空间中扩散，能量衰减很快，而且频率越高，衰减越严重。路径损耗大致与频率的平方成正比，28 GHz 的信号比 2.4 GHz 的在相同距离上多损耗约 20 dB，功率只剩百分之一。从发射到接收，这一路的功率得失都要算清，工程师把它叫作链路预算，而路径损耗正是这本账上最难平衡的一笔。面对它，两条路的应对截然不同。一条路正面硬扛：与其用一根天线向四周辐射，不如把几十上百个微小的天线单元排成阵列，分别控制每个单元的相位，让电磁波在目标方向上叠加、在其他方向上抵消，汇聚成一束波束，把损失的能量补回来，这就是相控阵。手机直连卫星是它的极端例子：卫星相当于把基站搬到几百公里高的轨道上，靠星上一面很大的相控阵，对准地面那部功率和天线都很小的手机。
...
一部 5G 毫米波终端，会在指甲盖大小的面积里集成上百个天线单元和相应的移相器、放大器，毫秒内就能把波束对准基站。这种集成度十年前还难以想象，是毫米波研究最活跃的方向之一。另一条路压根用不上这些。靠电池过活的物联网芯片没有功率预算，也不需要相控阵：它要连接的不过是几十米外的网关，传不远本来就不是问题，难的是每次发送要做到多省。力气全花在反方向：把整个收发机做得尽量简单、尽量小，把每次发送的能量降到最低，能不发就不发。同样一段距离，一边靠加大发射，一边靠少发、把电路做到最简。
...
信号穿过空间到达接收机时，往往只剩 -100 dBm，也就是 0.1 皮瓦。发送时是几瓦，接收时是飞瓦，相差十几个数量级。把这个几乎被噪声掩盖的信号从噪声中分辨出来，是低噪声放大器（LNA）的任务。两侧在这里遇到的是同一个限制：热噪声。要得到更低的噪声、更高的灵敏度，就得加大晶体管的偏置电流，而热噪声划定的下限，再精巧的电路也无法突破。毫米波芯片要 LNA 在几十 GHz 下仍压得住噪声，物联网芯片要它在纳安级电流下仍保得住灵敏度。一个被频率逼，一个被功耗逼，都只能在这条下限之上找一个能接受的折中。

无论收还是发，整条链路都需要一个稳定的本振，为信号确定载波频率。这本身是一门专门的学问，叫频率综合，核心是锁相环（PLL）。要做毫米波，本振得做到几十 GHz 还稳得住；要做低功耗，又得让它在几乎不耗电的条件下保持纯净。这个每个收发机都离不开的部件，同样被两头的需求往相反方向拉。

两条路各自走到尽头，都会撞上一堵墙。高频这条路的尽头是太赫兹，频率高到 300 GHz 以上，晶体管的增益已近枯竭，长期缺乏可用的有源器件，近年才有研究用标准 CMOS 工艺把收发机做到这个频段，将边界又推进一步。低功耗那条路的尽头正相反，一个比特究竟还能省到多低的能耗，至今没有定论。同一门射频，一边被推向更高的频率，一边被压向更低的功耗，就长成了今天这么宽的一个方向。
