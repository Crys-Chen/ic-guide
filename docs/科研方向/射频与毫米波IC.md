...
课程信号是三门的组合，模拟电子线路、电磁场与波、信号与系统。单是模拟电路好还不够，这个方向真正的分水岭在电磁场，觉得史密斯圆图和 S 参数是顺手的工具而不是天书，才算摸到门。日常一半在仿真，Cadence 里调晶体管，电磁仿真里抠走线和片上电感，版图提出寄生后推倒重来是常态；另一半在测试间，网络分析仪、频谱仪、毫米波探针台轮着用，校准没做对，一下午的数据全部作废，测量本身就是门手艺。性格上适合精打细算的人，这一行的思维方式是记账。高频路线算链路预算，每个 dB 的来去都要有交代；低功耗路线算能量预算，每个比特耗多少纳焦锱铢必较。两条路气质也不同，喜欢极限工程、想把频率推到晶体管增益尽头的去毫米波和太赫兹，享受在死约束里抠效率的去物联网低功耗。提醒两句。节奏被流片死死拽住，一轮反馈常以半年计，舞台是 ISSCC 和 RFIC；高频 debug 还带点玄学色彩，仿真和实测对不上是家常便饭，得有靠排查而不是靠运气把原因找出来的韧性。
