...
相控阵补回的是空间里损失的能量，可链路两端的器件本身，也被高频逼到极限。信号穿过空间到达接收端时，往往只剩 -100 dBm，也就是 0.1 皮瓦，和发射时的几瓦相差十几个数量级。要把这个几乎被噪声淹没的信号分辨出来，是低噪声放大器（LNA）的任务，而它能压到多低的噪声，被热噪声划下了死线：越想低噪声，就得越大的偏置电流，再精巧的电路也越不过这条下限，何况频率爬到几十 GHz，压住噪声本就更费劲。

发射端的功率放大器（PA）是另一处硬仗。要传得远就得输出大功率，可大功率把晶体管推进非线性区，冒出谐波、干扰邻道；要保持线性又得回退工作点，效率随之塌下来，一个 4G 基站的 PA 效率常年只有三四成，其余的电都成了热。就连给收发机定载波的本振，也得在几十 GHz 上稳得住，这是一门叫频率综合、核心是锁相环的专门学问。

这条路的尽头是太赫兹。频率高到 300 GHz 以上，晶体管的增益已近枯竭，长期找不到趁手的有源器件，近年才有研究用标准 CMOS 工艺把收发机做到这个频段，把边界又往前推了一步。

可另一大批芯片，追求的方向完全相反。它们根本不缺带宽：手腕上的手环、植入体内的起搏器、铺在田里的传感器，要连的不过是几十米外的网关，传得远不远从来不是问题，真正的考验是一颗纽扣电池要撑上十年。这一路算的不再是链路预算，而是能量预算——每发一个比特耗多少电，都要锱铢必较。

省电的第一招是干脆少发。靠电池过活的收发机大部分时间都在休眠，功耗压到几微瓦，只在该发送时醒来，数毫秒内把数据发完立即睡回去，平均功耗约等于占空比乘以峰值功耗，睡得越久就越省。第二招是把电路本身做到最简、最小，能省一级就省一级，而不是像毫米波那样堆上百个天线单元。
