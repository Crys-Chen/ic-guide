...
这个方向的研究对象，是天线和数字芯片之间那一整套模拟电路，合称收发机。接收时，微弱的信号先经低噪声放大器（LNA）放大，再混频、送进 ADC 变成数字；发射时反过来，功率放大器（PA）把信号推上天线；收发都靠本振（锁相环 PLL）提供准确的载波，到了毫米波再加一片相控阵前端。研究者要做的，就是把这些模块用晶体管搭出来、不断发明新结构，把噪声、效率、频率这些指标逼向物理极限。做出来的芯片，装进 5G/6G 基站和手机、汽车的毫米波雷达、卫星互联网终端，以及电池供电的物联网和可穿戴医疗设备——凡是要无线传信息的地方都少不了它，高端型号在国内至今高度依赖进口。
...
模拟电路学得踏实的人在这里会走得更深。射频和纯低频模拟不同，频段一上 GHz，走线本身就是电路的一部分，S 参数和史密斯圆图用着不别扭才算真正入门。日常大约一半在仿真器里追噪声指标、调 PA 偏置、搭相控阵阵列，另一半在测试间操作网络分析仪和频谱仪，毫米波的测量有一套专门的手法要学。流片半年一轮，仿真和实测对不上是常态，能追着不放、一层层往芯片里找原因，是这个方向最需要的习惯。
