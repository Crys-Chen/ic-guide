...
- **高频下"理想导线"假设崩塌**：进了 GHz，几毫米走线变成会移相的电感、平行走线寄生耦合漏信号、晶体管增益一路下滑，低频那套电压增益语言失效，得改用 S 参数和史密斯圆图重建直觉。
- **链路预算这道十几个数量级的鸿沟**：发射端 PA 几瓦，传到接收端只剩 0.1 皮瓦，每一分贝增益与损耗都要一笔笔记清攒够。
- **LNA 的噪声-功耗硬底线**：从噪声地板里把微弱信号捞出来要压低噪声系数，而这要更大偏置电流，热噪声划下的下限再巧的拓扑也绕不过。
- **PA 的线性-效率两难**：大功率要把晶体管推进非线性区会冒谐波污染邻道，想保持线性又得回退工作点、效率塌掉，4G 基站 PA 常年只有三四成。
- **毫米波路径损耗与相控阵**：路径损耗约与频率平方成正比，28 GHz 比 2.4 GHz 同距离多衰减约 20 dB，只能把上百个天线单元和移相器塞进指甲盖大小的模组拧出波束抢回能量，集成度极限尚无定论。
- **毫米波/太赫兹的频率天花板**：同一套收发机既是 5G 前端也是 77 GHz 车载雷达，再往上的太赫兹（300 GHz 以上）因晶体管增益枯竭、找不到趁手的有源器件而几近空白。
...
这个方向适合愿意把电磁场当第一性约束、对高频物理有兴趣的人。你得习惯一根几毫米走线在毫米波会变成天线、用 S 参数而非电压增益去描述放大器，还要忍住 LNA 噪声与功耗、PA 线性与效率这些物理划死的权衡。微电子本科最对口的切入口在模拟电路与电磁场这条硬件线。把模拟 IC 的噪声、差分对、偏置吃透才谈得上拿捏 LNA 和 PA，电磁场学扎实才看得懂路径损耗、相控阵和片上传输线，正是国产替代最缺人的一环。它的代价是节奏被流片死死拽住，一轮反馈常以半年计，最有代表性的舞台是 ISSCC 和微波侧的 IMS，受不了这种慢的人要想清楚。
