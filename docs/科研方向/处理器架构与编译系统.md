...
课程信号分两半。硬件这半看数字逻辑和计算机组成，学到流水线冒险时觉得有意思而不是背诵负担，是最直接的信号；软件这半看数据结构和 CSAPP 这类系统课，往编译器走还得对程序怎么一步步变成机器码有兴趣。日常在微电子各方向里最像程序员，几乎全天写代码读代码，写 RTL 或 Chisel，在 gem5、Verilator 里跑仿真数 cycle，或者给 LLVM、MLIR 写 pass，一篇论文背后常是几个月的实现加测量。它适合喜欢写代码、但不满足于调 API 的人，乐趣得来自搞清楚一段程序为什么慢，然后亲手把它变快，读 Transformer 时第一反应是这个矩阵乘要吃多少带宽，就是对的信号。还有一个隐性门槛是读大代码库的耐心，gem5、LLVM、香山都是百万行级的开源工程，进组第一件事往往是在别人的代码里游几个月泳。提醒两句。节奏比 AI 顶会慢，ISCA 和 MICRO 的审稿人只认带实现的结果，想法再好也得先复现 baseline 才有资格比较。好处是反馈不靠流片，改完代码当天就有数据，在硬件方向里算反馈最快的一档。
