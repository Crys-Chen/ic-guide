...
- **后摩尔时代性能从哪来**：Dennard 缩放与摩尔定律相继失效、主频钉死在 4 GHz，大模型算力需求还在指数攀升，而搬一次主内存够算几百次乘法，芯片大量时间在等而非在算，GPU 靠线程切换、TPU 靠脉动阵列各解一角，谁也没给出终极答案。
- **专用与通用的权衡（IC 切口）**：通用 CPU 塞满分支预测、乱序引擎、大缓存，跑神经网络时几乎全空转，DSA 拆掉只留矩阵乘电路能效高一两个数量级，但算法一变芯片可能就要重设计，这条最优边界一直在移动。
- **硬件与软件的边界在哪**：什么固化进硬件、什么交给编译器排定、什么留给运行时调度，同一块 GPU 换一套 KV Cache 调度策略 LLM 吞吐量就能差十倍以上。
- **要不要走出冯·诺依曼**：计算与存储分离的基本假设是否已成枷锁，神经形态、数据流、近存计算、存算一体能否从根本上消解数据搬运成本。
- **编译器如何驾驭新硬件**：从 LLVM 到 MLIR 的多层中间表示、TVM 自动调优、vLLM 式运行时调度，如何在保证语义等价的前提下把算法高效映射到不断变化的加速器上。
...
适合对"计算机为什么能快"真正好奇、愿意在算法和电路两个抽象层之间来回切换的人。读 Transformer 时第一反应是这个矩阵乘要吃多少带宽、KV Cache 怎么放，就是对的信号。微电子本科在这条线上有天然优势，数字电路、计算机组成、时序的底子让你读得懂 RTL 和微架构，自然偏向架构与 DSA 一侧。诚实地说，这里节奏比 AI 顶会慢，一篇论文要靠扎实的 RTL 实现撑着，奖励把问题挖深而非快速迭代。
