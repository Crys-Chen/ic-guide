...
架构研究离不开编译器，<u>一块新芯片没有配套软件栈，只是一块昂贵的硅片</u>。编译器的核心工作是**调度**，决定指令以什么顺序执行、数据从哪里取、什么时候取。调度有两种基本哲学。**静态调度**在编译期把一切安排好，生成的指令序列运行时无需硬件再做决策，是 **VLIW**（Very Long Instruction Word，超长指令字）这类架构的基础；**动态调度**把决策权留给硬件，处理器在运行时按实际情况乱序执行，能应对编译期无法预知的变化，是现代高性能 CPU 的标配。
