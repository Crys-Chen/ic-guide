...
- **算力与带宽的剪刀差**：浮点算力以百万亿次计，搬一次主内存的数据却够算几百次乘法——芯片大量时间在等而非在算。是学 GPU 用海量线程切换隐藏延迟，还是学 TPU 脉动阵列让权重只取一次反复复用，抑或另寻第三条路？
- **专用与通用的永恒权衡**：通用 CPU 塞满分支预测、乱序引擎、大缓存，跑神经网络时几乎全空转；DSA 拆掉这些只留矩阵乘电路，能效高一两个数量级，代价是算法一变芯片可能就要重设计。这条最优边界随算法迭代持续移动，没有终点。
- **硬件与软件的边界在哪**：什么计算该固化进硬件、什么交给编译器在运行前排定、什么留给运行时动态调度？同一块 GPU 换一套 KV Cache 调度策略，LLM 吞吐量能差十倍以上——这条边界本身就是研究对象。
- **后摩尔时代的性能从哪来**：Dennard 缩放与摩尔定律相继失效、主频钉死在 4 GHz，而大模型算力需求还在指数攀升。当"力大砖飞"不再奏效，架构的精细设计如何重新撑起性能曲线？
- **ISA 与微架构的设计空间**：x86 的兼容包袱、ARM 的精简取舍、RISC-V 的开源红利之外，分支预测命中率、预取深度、缓存替换（LRU vs RRIP）、存储层次每一级的协议，如何在功耗约束下逼出更高真实性能？
- **要不要走出冯·诺依曼**：计算与存储分离的基本假设是否已成枷锁？神经形态、数据流、近存计算（NMP）、存算一体（CIM）能否从根本上消解数据搬运的成本？
- **编译器如何驾驭新硬件**：从 LLVM 到 MLIR 的多层中间表示、TVM 的自动调优、vLLM 式的运行时调度——如何在保证语义等价的前提下，把算法高效映射到不断变化的加速器上？
...
这个方向最适合那些对"计算机为什么能快"这个问题感到真正好奇的人——不满足于"因为时钟频率高"这种答案，而是想弄清楚给定一批晶体管，怎样排布才能在物理约束下算得最快、最省电。如果前面那三个矛盾——算力追不上带宽、专用与通用的拉扯、硬件与软件边界的反复移动——读起来让你觉得"这才是有意思的问题"，那你大概率会在这里待得住。它的魅力恰恰在于：摩尔定律红利耗尽之后，性能不再自动到来，每一分提升都得靠设计去抠，这是 Hennessy 和 Patterson 所说"架构新黄金时代"的真正含义。

判断自己合不合适，有两个很直接的信号。其一，学 Verilog 或 Chisel 时你会忍不住想"这段代码综合出来的电路长什么样"，而不只是把它当描述语言用；其二，读 Transformer 论文时你的第一反应是"这个 attention 的矩阵乘要吃多少带宽、KV Cache 怎么放"，而不只是"精度涨了几个点"。这两种直觉——把算法落到电路、把电路反推回算法——正是这个方向研究者每天在做的事。它要求你愿意同时站在两个抽象层次上来回切换，而不是只守着其中一层。

从本科背景看，这个方向对 EE/微电子 和 CS 两类人都友好，但入口不同。EE/微电子 的同学有天然优势：数字电路、计算机组成、时序与信号完整性的底子，让你能真正读懂 RTL 仿真和微架构里那些分支预测、缓存替换的细节，自然偏向架构与 DSA 一侧。CS 背景的同学若补上 Verilog 和流水线设计，从编译器一侧（LLVM/MLIR/TVM、vLLM 这类运行时调度）切入会更顺，而 RISC-V 的开源把芯片创新门槛降到了学术组也能参与的程度，正好给了 CS 出身的人动手做真硬件的机会。两条路最终会在"算法如何高效映射到硬件"这个交汇点上相遇。

发表阵地分两侧：架构侧主要是 ISCA、MICRO、HPCA，编译侧是 ASPLOS、CGO、PLDI（ASPLOS 的名字里同时带着 Architecture、Programming Languages 和 Operating Systems，恰好说明这个方向的交织本质）。要有心理准备：和 AI 顶会比，这里的节奏偏慢，一篇论文往往要扎实的 RTL 实现加量化实验撑着，不是改几行就能刷出来的。如果你享受把一个问题挖深、用真实硬件数据说话，而不是追求快速迭代，这个节奏会让你觉得踏实而非煎熬。
...
<div class="dm-wrap chip-block">
  <div class="dm-row dm-device"><span class="dm-lbl">国内</span><span class="dm-chips">
    ...
    <a class="dm-chip" href="https://www.ime.ac.cn" title="存算一体与新型计算架构 · 国产 EDA">中科院微电子研究所</a>
    <a class="dm-chip" href="https://www.zhejianglab.org" title="智能计算 · 算力系统与异构架构">之江实验室</a>
