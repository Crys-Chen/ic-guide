...
课程信号集中在两门。模拟电子线路学得有感觉，看到差分对和反馈环路觉得优雅而不是折磨，是第一信号；信号与系统是第二信号，采样、噪声、频域这套语言以后天天要用，再加点概率和随机过程的底子，才能跟热噪声和失配讲道理。日常是典型的慢手艺。大部分时间在 Cadence 里调器件尺寸、跑 SPICE、看波形，一轮一轮逼近八边形的某个角；版图常要自己画，失配和寄生就藏在那几微米里；然后等几个月一轮的流片，芯片回来后焊板子、搭测试，实测掉了两位有效位就回实验室查为什么。性格上适合手艺人型的，沉得住气，对细节有洁癖，相信经验复利。这一行越老越吃香，踩过的坑就是壁垒，不会被一波新工具洗掉，AI 辅助设计喊了几年也只是加速器。提醒两句。成长曲线慢，前几年最熬，论文分量永远压在流片实测而非仿真图上，顶会是 ISSCC；爱写 RTL、想靠快速迭代和聪明取胜的人，在这里讨不到好。
