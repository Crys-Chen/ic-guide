...
设计连接物理世界和数字世界的接口电路。声音、图像、无线电都是连续的模拟量，进出数字芯片要经过 ADC、锁相环、SerDes 这些混合信号模块，每一块 SoC 都离不开它们。
...
适合沉得住气的手艺人。模拟电子线路和信号与系统是根基，觉得差分对和反馈环路优雅而不是折磨的人，适合走这条路。日常就是在 Cadence 里调尺寸、跑仿真、画版图，一轮轮逼近物理极限，然后等流片、回片测试，论文的分量压在实测数据上。这一行经验复利，越老越吃香，但成长曲线也慢，前几年最熬，想靠快速迭代和聪明取巧的人讨不到好。
