...
- **噪声-功耗-精度的三角博弈**：热噪声来自电阻与晶体管中电子的随机热运动，理论上无法消除——想压低它就得付出更大偏置电流或更大电容的代价，意味着更多功耗或更大面积。Walden FoM 散点图几十年来只缓慢移动，正因每一步都在与热力学基本定律抗争：在给定工艺下，架构还能把 ADC 往那条物理极限推多近？
- **速度与精度的此消彼长**：ADC 每次采样都需要一定的建立时间，想更快就得接受更多误差，想更高精度就得放慢速度。SAR 用二分搜索在低功耗场景里高效取胜，ΔΣ 把量化噪声整形到高频再用数字滤波器压制——两者只是在"速度·精度·功耗"三角里选了不同的生存位置。面对一个具体需求，哪种架构（乃至两者的杂交）才是当下最优解？
- **超高速 SerDes 的信号还原**：当数据中心要芯片间每秒搬运数百太比特，物理约束就从实验室问题变成产业瓶颈。224 Gbps PHY 要在损耗 40 dB、多处反射的铜线上恢复信号，靠发送端多抽头 FFE 预补偿、接收端 CTLE/DFE 与 CDR 逐级还原。速率每三年翻倍（56→112→224→448）且从不是简单缩放——下一代又有哪些电路节点必须推倒重审？
- **PLL 相位噪声的时域版权衡**：每块数字芯片里的 PLL 从低频参考时钟合成 CPU 主频，相位噪声（时钟不纯净度）直接吃掉时序裕量；5G 基带本振 PLL 相噪每差 1 dB，系统误码率就显著恶化。这是换了物理量、结构却与 ADC 的 noise-power 矛盾完全相同的问题——如何在更低功耗下把相噪再压一截？
