...
- **PLL 相位噪声的时域版权衡**：PLL 把慢的参考时钟倍频成芯片主频，可时钟边沿总会忽早忽晚地抖，这点抖动（相位噪声）在数字芯片里压缩信号稳定的余量、拉低主频，在 5G 收发机里让符号糊成一团、抬高收错的概率。这还是 ADC 那个 noise-power 矛盾，只是换到了时间轴上，如何在更低功耗下把抖动再压一截？
