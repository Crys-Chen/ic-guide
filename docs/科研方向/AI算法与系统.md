...
数据层面，模型最终学到什么不靠架构、靠数据。GPT-4、Llama 3 量级的训练数据在 10-15 万亿 token，基本把开放互联网的高质量内容训了一遍。如今，互联网公开数据已经快用光了。接下来获得数据的方法只有两条，一是**合成数据**：让强模型生成新数据训练后续模型，比如 Tesla 的 FSD也用模拟器生成小孩冲马路、暴雨夜驾这类极端场景来训练自动驾驶。另一条是**专家精选数据**，比如科学论文、专业的推理过程标注。
