...
适合真心对“让模型更聪明、更高效”有热情的人。这个方向是整个集成电路领域最贴近 AI 的。如果做纯算法，那迭代节奏极快，成果挂 arXiv，论文短平快，今天的 SOTA 三个月后就被刷掉，跟得上会兴奋，跟不上会焦虑。日常就是写 PyTorch、改模型、在 GPU 集群上排队跑实验。如果结合系统和硬件架构，那就是 AI Infra 了，非常适合微电子出身的同学。系统侧，量化、推理优化这些贴硬件的活，我们能从访存带宽理解它为什么省，纯软件出身的人未必有这个直觉；越往纯算法和理论走，数学要求越重，得下决心补，靠硬件直觉硬扛不过去。
