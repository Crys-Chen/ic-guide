...
适合觉得“换一种计算方式”比沿着旧路再挤一点性能更有意思的人。切口有两个，模拟电路扎实的去做神经元电路和脉冲路由，对器件物理有兴趣的去研究忆阻器，让它的漂移对齐 STDP、把学习直接搬进硬件本身。日常横跨两个领域，白天调电路或器件，晚上读 SNN 训练的算法论文，社区偏小且分散，得习惯两边来回切换。还要有心理准备，SNN 的精度至今落后主流神经网络几个百分点，做这个方向看重的是能效那几个数量级的差距，不是榜单上的名次。
