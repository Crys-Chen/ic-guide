...
- **相干与门保真度撞纠错阈值**：超导比特相干只有几百微秒、单次操作仍有千分之一出错，物理错误率压不到表面码阈值以下纠错就赚不回来。
- **可扩展与工艺一致性**：保护一个逻辑比特要数十到数百个物理比特，实用容错机是百万级规模，约瑟夫森结的尺寸与材料纯度良率难以对接可量产工艺。
- **控制线瓶颈与低温 CMOS（IC 切口）**：上百条微波线从 20 mK 引到室温每条都往冷端传热，把控制电路下沉到 4 K，但标准 CMOS 一冷迁移率和阈值电压全漂移、器件模型集体失效要从头标定。
- **超导之外的技术路线**：离子阱出错率低却操作慢几个数量级，中性原子阵列、硅量子点、光量子各自的扩展瓶颈仍未定胜负。
- **量子算法与量子版 EDA**：把算法编译映射到芯片连接拓扑、压操作次数控错误积累，和经典逻辑综合同构，但在 NISQ 协处理器范式下要既贴硬件又有实用价值仍没解。
...
这个方向适合被极限条件工程吸引、愿意一只脚踏在量子力学和固体物理上的人，完全想回避物理的人不适合。微电子本科的切口很具体，不拼量子比特本身，而是做 4 K 低温 CMOS 控制芯片和微波测控链路。标准 CMOS 一冷到 4 K 器件模型全失效要从头标定，你能在这里贡献低温器件建模、射频测控集成和读出链路，正对接量旋、本源量子这类厂商的测控系统。诚实提醒一句，容错量子机最乐观也要 2035 年以后，需要长线思维和补物理课的决心。
