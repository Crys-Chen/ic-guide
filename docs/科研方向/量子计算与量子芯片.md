...
问题在于，普通 LC 谐振器的所有相邻台阶间距完全相同，意味着 0→1、1→2、2→3 的共振频率一模一样。你打一个想激励 0→1 的微波脉冲，1→2 和更高的跃迁也同时响应，系统根本不会停在最低两个台阶之间，没法把它当成干净的二态系统来用。解决办法是把电感换成**约瑟夫森结**，两片超导体中间夹一层纳米厚绝缘层构成的非线性电感，它让台阶间距不再均匀：0→1 的共振频率和 1→2 不同。这样打一个精确调到 0→1 频率的脉冲，只有最低两个台阶响应，其余台阶无动于衷。这两个台阶就是量子比特的 0 和 1。
