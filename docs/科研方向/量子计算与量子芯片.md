...
把这些原理变成真实器件，目前主流路线是超导量子比特。对学过模拟电路的人，理解它最快的方式是从 LC 谐振回路出发。普通 LC 谐振器各个谐振频率之间的间隔均匀，打一个微波脉冲会同时激励好几个，没法只操控其中最低的两个。把电感换成**约瑟夫森结**——两片超导体中间夹一层纳米厚绝缘层构成的非线性电感——谐振频率的间隔就不再均匀了：最低两个频率之间的间距和其他的不一样，可以用一个特定频率的微波脉冲只操控这两个。超导量子比特就是这样工作的：用电路最低的两个状态分别编码 0 和 1，用微波信号操控和读取，不是电压的高低。
