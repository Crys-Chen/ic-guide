...
- **相干时间太短**：20 mK 下量子态也只能维持几百微秒，像谐振器 Q 值一样持续衰减，这段时间能做多少操作直接框死算法复杂度。
- **门保真度撞纠错阈值**：最好的超导比特单次操作仍有千分之一出错，跑数百万次必然累积失效，物理错误率压不到表面码阈值以下纠错就赚不回来。
- **可扩展性的代价**：保护一个逻辑比特要数十到数百个物理比特，实用容错机意味着百万级规模，芯片制造与互连密度已逼近经典先进制程，良率和串扰随时拖垮它。
- **控制线瓶颈与低温 CMOS（IC 切口）**：每个比特要多条微波线从 20 mK 引到室温、每条都往冷端传热，把控制电路下沉到 4 K 是出路，但标准 CMOS 一冷到 4 K 迁移率和阈值电压全漂移、器件模型集体失效要从头标定。
- **约瑟夫森结的一致性**：非线性能级靠两片超导体夹纳米绝缘层的结实现，结尺寸、材料纯度与良率难以对接可量产工艺。
- **量子版 EDA**：把算法映射到芯片连接拓扑、压操作次数、控错误积累，和经典逻辑综合高度同构，但在 NISQ 协处理器范式下既要贴硬件又要有实用价值仍没解。
...
这个方向适合被极限条件工程吸引的人。核心社区在物理（APS March Meeting、PRX Quantum），你得有一只脚踏在量子力学和固体物理上，叠加、测量坍缩、超导与约瑟夫森结的基本图像是入场券，完全想回避物理的人不适合。微电子本科的差异化切口非常具体，不去和物理系拼量子比特本身，而是从工程硬件侧进场。做 4 K 低温 CMOS 控制芯片、20 mK 到室温的微波测控链路、约瑟夫森结的微纳加工。标准 CMOS 一冷到 4 K 器件模型就全部失效需要从头标定，这正是 IC 学生最能立刻发力的地方，成果常落在 ISSCC 这类固态电路阵地。诚实提醒一句，容错量子机最乐观也要 2035 年以后，读博期间看不到落地变现，需要长线思维和补物理课的决心。
