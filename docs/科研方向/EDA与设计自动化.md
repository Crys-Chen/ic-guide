...
- **全流程都是 NP hard 优化**：从综合、验证到布局、布线、时序、寄生提取、热分析，每一步搜索空间都是天文数字、目标相互牵制，模拟退火和力导向这些人写的启发式在工艺趋严、设计变大时越来越吃力。
- **时序收敛磨人**：布完线发现路径超时，局部重布又牵动别处，工程师常在布局、时序、布局里迭代数周，能否提前预测违例、早期就避免反复返工还没解。
...
- **3D 与热**：芯片走向 3D 堆叠后布局布线要跨 die 协同，夹在中层的热无处可逃，热仿真与热感知设计怎么从配角变成一等公民是新难题。
...
这个方向适合喜欢把芯片设计当数学题解的人，对组合优化、图算法、机器学习有兴趣，又希望自己写的算法不停在论文里而是被工具厂集成进流程。微电子本科在这里有一种纯算法出身的人补不上的领域直觉。你更容易明白为什么某个优化目标重要、为什么时序收敛这么难、为什么模拟 EDA 至今啃不动，这种直觉在提出有价值的问题时很值钱。入门门槛是弄懂综合、布局、布线、STA 这条流水线每一步在干什么，跑一遍 OpenROAD 的完整数字后端、再拿 CircuitNet 做几个实验基本就迈过去了。诚实提醒一句，这是个会议驱动的社区（旗舰 DAC、ICCAD），评审只认在标准 benchmark 上把指标实打实推进的工作，纯讲故事很难过关。
