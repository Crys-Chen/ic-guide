...
AI for EDA 是过去五年增长最快的子方向，核心逻辑是：EDA 中的优化问题大多是 NP 难的组合优化，传统启发式算法在设计规模指数级增长后逐渐失效，而深度学习恰好擅长从海量历史设计数据中提炼规律。当前最活跃的分支包括：强化学习做布局（AlphaChip 把宏单元摆放建模为序列决策，已用于 TPU v5 全流程）、图神经网络做时序与拥塞预测（布线前就能预警哪些区域会有问题，NVIDIA 和 Synopsys 均已集成进商业工具）、大语言模型做 RTL 生成（ChipNemo、RTLCoder、VerilogCoder 等把自然语言需求转成可综合 Verilog）。开源研究平台方面，DARPA 资助的 [OpenROAD](https://github.com/The-OpenROAD-Project/OpenROAD) 提供完整的数字后端流程（综合到 GDSII），北大林亦波团队发布的 [CircuitNet](https://circuitnet.github.io/) 是国内首个面向 AI for EDA 的大规模开源数据集，HKUST 谢知遥团队是学术界在 LLM for EDA 上最活跃的组之一（ASPLOS 2026 Best Paper）。
