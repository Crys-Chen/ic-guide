...
芯片设计的规模，大到没法用直觉去想象。一块 Apple M4 大约有 280 亿颗晶体管，挤在 300 平方毫米的硅片上——等于要在指甲盖大的地方，把几百亿个零件摆好、连对。这么大的东西，没人能一个晶体管一个晶体管地画。工程师能做的，是用**硬件描述语言（HDL，最常见的是 Verilog）**在高层描述"我要什么逻辑"，至于"怎么把这份意图变成能送进晶圆厂的版图"，全部交给软件。从 HDL 到版图，中间要走逻辑综合、布局、时钟树、布线、寄生提取、静态时序分析等数十步，每一步都在啃亿级规模的图或几何优化。这整套自动化流程就是 **EDA（Electronic Design Automation）**，堪称"芯片背后的芯片"——没有它，现代芯片根本造不出来。

这里有个常被 CS 同学误解的点：HDL 看着像写程序，其实根本不是。你写的每一行综合出来都是实打实的电路，它们**同时**工作，而不是像软件那样一行行顺序执行——是用代码描述并行的硬件，而非编排一段流程。而且"人用什么层次描述硬件"这件事，几十年来一直在往上抬：从手写 RTL，到用 **Chisel** 这类语言借软件的抽象力去生成 RTL，再到 **HLS** 干脆写 C、让工具综合成电路。每抬高一级，人就把更多苦活推给机器——记住这条线，它后面还会再抬一次。

这套自动化流程，本质是在求解一连串 **NP hard 甚至更难的优化问题**，而且远不止"布局布线"这一块。往前看有逻辑综合、功能验证，往后看有布局、布线、时序收敛、寄生提取、热分析、可制造性（光刻与 DFM），每一步都是自成一体的难题。拿最经典的布局来说：把几亿个逻辑单元摆到芯片平面上，既要让关键路径的连线短（管时序）、又要拥塞低（管布通）、还要电源压降均匀，这些目标相互拉扯，而可能的摆法是个天文数字。几十年来，工具靠**人想出来的启发式**（模拟退火、力导向）在合理时间里找个"够好"的解，可设计越做越大、工艺越来越严，这些老办法越来越吃力。时序收敛尤其磨人：布完线发现一条路径超时，局部重布又牵动别处，工程师常在"布局—时序—布局"的循环里耗上几周。
...
机器学习闯进 EDA，正是冲着这些痛点来的，如今已是这个方向最热的战场。最有名的一仗是布局：2021 年 Google DeepMind 在 *Nature* 上发表 **AlphaChip**，把"哪个模块摆哪里"建模成一盘棋，让强化学习智能体反复试错、自己悟出摆法。在 TPU 的实际设计里，它几个小时给出的布局胜过人类工程师几周的手工优化，而且已经用在量产芯片上。但 AI 远不止于布局，它正渗进流程的每一步：**图神经网络**提前预测哪里会拥塞、哪条路径会超时，让工程师早早就改、不必等到最后返工；**机器学习**扫一遍版图，就能挑出将来光刻会出问题的"热点"；到了最上游，**大语言模型**开始把自然语言需求直接写成可综合的 RTL，这恰好把开头那条"抽象层"又往上抬了一级——从 HDL 抬到了人话。

AI 能在 EDA 里使上劲，靠两个底气。一是 EDA 的核心对象天生就是**图和搜索**：网表是图、布局是图、时序路径是图，正对图神经网络和强化学习的胃口。二是有了 **CircuitNet**（北大林亦波团队）这类开源数据集，模型才头一次有了足够多、足够规整的样本可学。不过别把话说满：AlphaChip 那样真进量产的还是少数，大量 AI for EDA 仍停在论文和实验阶段，离"换掉传统工具"还远着。

可 AI 这条主线，有一处怎么也撞不进去，那就是模拟电路的设计自动化。数字 EDA 能让机器学，是因为它有"满足时序"这么一把清晰、可量化的标尺：好不好，数字说了算。模拟没有这把尺。它的指标是一整张相互牵制的清单：增益、带宽、噪声、线性度、摆幅、功耗、稳定性，改好一个往往牺牲另几个，根本没有单一目标可优化。更糟的是，模拟极度依赖工艺仿真模型（SPICE），而它在高频下误差不小，仿真和真实流片对不上，机器连个可信的"标准答案"都拿不到，自然学不出规律。这正是为什么数字 EDA 已相当成熟、模拟 EDA 却至今大半靠老师傅手工调，成了这个领域最难啃的骨头。
...
还有一类难题不是 AI 学不学得动，而是芯片本身变了样，逼着 EDA 长出新本事。摩尔定律放缓后，芯片不再满足于在一个平面上铺，而是开始往上**叠**——多颗裸片用 TSV、混合键合堆成 3D，或拆成 chiplet 再拼起来。这一叠，EDA 的难题就从二维升到三维：布局布线要**跨 die** 协同，版图规划得管好垂直方向的对齐和互连。最棘手的是**热**：几层裸片叠在一起，中间那层产生的热量几乎无处可逃，温度一高，时序和可靠性全乱。于是**热仿真、热感知设计**从过去的边角配角，一跃成了 3D 时代必须摆上台面的一等问题。
...
最后值得一提的是这个方向的分量。EDA 是整条芯片产业链里最典型的卡脖子环节：Synopsys、Cadence、Siemens 三家美国公司握着全球八成以上的市场，2019 年那道对华为的禁令，几乎一夜之间让海思失去了推进先进制程的工具。但反过来看，这也意味着一个更好的算法真能撬动整个行业——EDA 是少数"做出来就能改变游戏规则"的基础设施级方向。

### 核心研究问题

- **NP hard 优化（贯穿全流程）**：从综合、验证到布局、布线、时序、寄生提取、热分析、可制造性，每一步都是 NP hard 甚至更难的优化，搜索空间天文数字、目标相互牵制。模拟退火、力导向这些人写的启发式在工艺趋严、设计变大时越来越吃力，如何让算法继续扩展？
- **时序收敛（timing closure）**：布完线发现路径超时，局部重布又牵动别处，工程师常在"布局—时序—布局"里迭代数周。能否提前预测违例、在早期就避免反复返工？
- **AI for EDA（当前主战场）**：强化学习做布局（AlphaChip）、图神经网络预测拥塞与时序违例、ML 检测光刻热点、LLM 把自然语言写成 RTL——AI 正渗进整条流程。EDA 的对象天生是图与搜索、又有了 CircuitNet 这类开源数据，这是 AI 切入的底气；但它能多大程度替代传统启发式、又会在哪里失灵？
- **模拟 EDA 的不可学性**：模拟没有"满足时序"这样的单一标尺，增益/带宽/噪声/线性度/功耗相互牵制，又高度依赖高频下不准的 SPICE 模型，机器拿不到可信标签——数字成熟而模拟至今手工。如何建立可靠的模拟综合？
- **3D 与热**：芯片从平面走向 3D 堆叠（TSV/chiplet），布局布线要跨 die 协同，叠层之间的热又无处可逃，热仿真与热感知设计如何从配角变成一等公民？
