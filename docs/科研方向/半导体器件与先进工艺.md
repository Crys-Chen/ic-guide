...
- **二维半导体能不能量产（IC 切口）**：硅切到 1-2 nm 表面长满悬挂键、迁移率塌掉，MoS₂、WSe₂ 天生薄又不塌，但晶圆级单晶生长、CMOS 后端兼容、欧姆接触全是开放问题，是最大的一类课题组方向。
...
适合对物理图像本身有执念的人。看到 DIBL 脑子里浮现的是漏极电场怎么把势垒一点点拉低，看到 RRAM 跳变会一路追到氧空位排成细丝那一层才肯罢休。微电子本科正是这条线的主力，半导体物理、固体物理底子扎实就能直接上手。想动手就进超净间生长二维材料、做新型存储器件、操作 ALD、在探针台量 I-V 和耐久性，这是中芯国际、长鑫存储这类代工存储厂和 TetraMem 这类新型存储公司最缺的人。想偏理论就用 Sentaurus TCAD 跑器件、写统计模型啃 RRAM 的变异性。代表舞台是 IEDM。诚实地说，一轮工艺动辄数周、失败重来是常态，二维材料离量产还有 5-10 年，它奖励的是对单个物理机制的耐心深挖而非迭代速度。
