...
<div><svg viewBox="0 0 880 250" style="width:100%;max-width:860px;display:block;margin:1.5em auto;font-family:system-ui,-apple-system,sans-serif">
  ...
  <text x="705" y="32" text-anchor="middle" font-size="13" font-weight="700" fill="#1E293B">不数电荷的四种物理量</text>
  <rect x="552" y="46" width="150" height="72" rx="5" fill="#EFF6FF" stroke="#3B82F6" stroke-width="1.5"/>
  <text x="627" y="62" text-anchor="middle" font-size="10" font-weight="700" fill="#1E40AF">RRAM 电阻态</text>
  ...
  <text x="627" y="142" text-anchor="middle" font-size="10" font-weight="700" fill="#166534">MRAM 磁化方向</text>
  ...
  <text x="787" y="142" text-anchor="middle" font-size="10" font-weight="700" fill="#5B21B6">FeRAM 极化翻转</text>
  ...
  <text x="705" y="222" text-anchor="middle" font-size="10" fill="#475569">都把一个双稳态物理量当作 0 和 1</text>
  <text x="705" y="236" text-anchor="middle" font-size="10" fill="#94A3B8">速度、功耗、耐久怎么打平还没定论</text>
</svg></div>

### 核心研究问题

- **二维半导体能不能量产（IC 切口）**：硅切到 1-2 nm 表面长满悬挂键、迁移率塌掉，MoS₂、WSe₂ 天生薄又不塌。晶圆级单晶生长、CMOS 后端兼容、欧姆接触全是开放问题，是最大的一类课题组方向。
- **新型非易失存储靠哪种物理顶上**：DRAM 电容存不住电荷、3D NAND 堆过 200 层压不住应力。RRAM 用电阻态、PCM 用相态、MRAM 用磁化、FeRAM 用极化翻转，哪种能在速度、功耗、耐久、保持之间打平还没定论。
- **器件随机性是缺陷还是存算一体的特性**：RRAM 细丝在哪成形带着随机性，放进阵列就是良率分布问题。可连续可调的电阻态又天然衔接存算一体，一大批课题组在这上面做忆阻器与神经形态。
- **先进晶体管结构还能救几代**：沟道一短漏极电场就侵入栅极、提前拉低势垒（DIBL）。FinFET 三面包、GAA 四面环绕、CFET 再垂直堆叠，靠几何把电子管住的路还能续几代。
- **铁电与先进工艺的可靠性**：FeFET、高κ 金属栅、二维材料接触、先进铜互连各有各的失效机制。ALD 这类工艺要在原子层尺度把界面缺陷和可靠性压下去。
...
适合真心喜欢物理的人。量子力学、固体物理、半导体物理这条线学得顺是基本盘，因为这个方向做的就是在物理机制层面找出路。日常分两种，做实验的泡超净间和探针台，长薄膜、做器件、量曲线，一轮工艺几周起步；做仿真的用 TCAD 跑器件、建模型，不用进超净间。共同点是慢，成果以年计，它奖励把一个机制刨到底的耐心，指望快速反馈的人会很难受。
