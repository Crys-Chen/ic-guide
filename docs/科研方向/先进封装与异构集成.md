...
适合愿意把分离的硅缝成一个整体、能在良率带宽能耗散热之间同时算账的系统型选手。微电子本科最稳的切入口在硬件那一侧。想动手就进洁净间做 TSV、微凸块、混合键合，量热阻跑温循可靠性，这是长电、通富微电这类封测厂最缺的人。想算就用 HFSS、COMSOL 把信号完整性和热场算清楚，或者把多 Die 时序、布图、热感知布局做成图算法去 EDA 厂落地。代表舞台是 ECTC 和 DAC 两侧。诚实地说，工艺师兄和布图师姐读的论文几乎不重叠，入行前得先想清楚自己往工艺还是设计自动化哪边靠。
