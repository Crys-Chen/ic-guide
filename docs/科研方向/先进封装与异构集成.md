...
- **拆到多碎才划算**：大芯片良率随面积指数级崩塌，Chiplet 拆得越小废品越少、拆得越多互联代价越高，那个甜点位置至今没有定论。
- **填平出边界的鸿沟**：信号一越出芯片边界，延迟翻倍、每比特能耗高一个量级，从 2.5D 转接板到 Cu-Cu 混合键合（<1 µm 间距），如何把 Die 间互联推到接近片内。
- **垂直堆叠的散热（IC 切口）**：3D 直接键合后功率密度可超 1000 W/cm²，上层 Die 堵死下层热路，不拆封装把热导走仍是硬骨头。
- **跨厂 Chiplet 的标准互通**：UCIe 想让不同厂家的计算核、HBM、加速器在同一封装里像单芯片一样对话，物理层与协议层的统一今天还远未完成。
- **已知良品（KGD）测试**：坏 Die 混进封装才暴露就连累整体报废，如何在裸片阶段完成全功能筛查是良率经济的命门。
...
适合喜欢把分离的硅缝合成一个整体、能在良率、带宽、能耗、散热之间同时算账的系统型选手。微电子本科最稳的切入口在硬件那一侧。工艺侧进洁净间做 TSV、微凸块、混合键合，量热阻、跑温循可靠性。仿真侧用 HFSS、COMSOL 把信号完整性和热场算清楚。EDA 侧则把多 Die 时序、布图、热感知布局做成图算法和优化问题。把脚下那块硅的物理极限始终记在心里，正是这里最缺的本事。诚实地说，工艺师兄和布图师姐读的论文几乎不重叠（ECTC 与 DAC 两侧），入行前得先想清楚自己往哪边靠。
