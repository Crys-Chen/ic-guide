...
- **良率经济学的拆分边界**：一整片大芯片要所有功能区同时合格，良率随面积指数级崩塌——到底该拆成多碎的 Chiplet、哪些功能各走哪种工艺（N3 逻辑 / N7 I/O / 自有 DRAM），才能在"拆得越小废品越少"和"拆得越多互联代价越高"之间找到那个甜点？晶圆级整片不切的另一极端（WSE-3 靠 reticle stitching + 冗余架构）又值不值得？
- **填平"出边界"的鸿沟**：信号一旦越出芯片边界，延迟翻倍、每比特能耗高出一个量级——从 2D 绕基板走线、2.5D 硅转接板（CoWoS/EMIB）、3D TSV 堆叠，一路逼到 Cu-Cu **混合键合**（<1 µm 间距、近片内密度），再到 2.5D+3D 合体的 **3.5D**，如何把 Die 间互联密度与能效一路推到接近片内，让"分开制造"不再付出"合并使用"的代价？
- **垂直堆叠下的散热**：3D 直接键合把芯片叠起来后，功率密度可超 1000 W/cm²、高过喷气发动机燃烧室——上层 Die 把下层的热路彻底堵死，如何在不拆封装的前提下（微流道、TIM、嵌入式冷却）把这些热导走？
- **跨厂 Chiplet 的标准化互通**：UCIe 想让来自 AMD 的计算核、三星的 HBM、台积电代工的加速器在同一封装里像单芯片一样直接对话——物理层与协议层怎样统一，才能在通用性和极低 die-to-die 开销之间不偏废？这件事今天还远未完成，是本方向最核心的开放前沿。
- **已知良品（KGD）测试**：坏 Die 一旦混进封装才暴露，整个高价值封装体连带报废——如何在裸片阶段就完成全功能筛查，把"打扫干净屋子再请客"真正做到位？
- **基板材料换代**：ABF 有机基板已成供应链瓶颈，玻璃基板介电损耗更低、线宽更细、CTE 更贴近硅——但玻璃质脆，激光钻微孔时如何不崩裂地打出高密度通孔？
- **光电共封装（CPO）**：传统光模块插在交换机端口，光电转换每过一次外部模块就烧掉一截插入损耗——把光引擎与交换 ASIC 直接同封装、让转换在芯片旁边完成，如何在数据中心网络层把"通信单元尽量近"做到极致？
- **异质材料的应力与污染**：把 GaN、InP、GaAs 等 III-V 材料拉进硅封装能各取所长（射频放大、大功率开关、激光发射），但 CTE 严重失配会在温度循环中撕裂界面，砷磷元素又会污染 CMOS 产线——共封装的热-机械应力与工艺隔离该如何设计？
...
这个方向真正吸引人的，是它逼你同时在四种语言之间切换：良率经济学、电磁、传热、和 EDA 算法。一块 Chiplet 拆得多碎、走哪种工艺、互联用 2.5D 转接板还是 Cu-Cu 混合键合——每个决定都在良率、带宽、能耗、散热之间做权衡，没有任何一项能单独最优。如果你喜欢的是"把分离的硅缝合成一个整体"这种系统级问题，喜欢在一张白板上同时画热路、信号路和成本账，这个方向会让你有大量施展空间；它也是少有的让物理直觉与工程取舍真正连续打通的领域。
...
发表阵地也随这三侧分流，提前看清能少走弯路。偏工艺与集成的人，主场是 **ECTC**（电子封装领域的奥运会）、**IEEE 3DIC**，重器件级突破则冲 **IEDM**，期刊落在 **IEEE T-CPMT**、**IEEE EDL**；偏 EDA 与物理设计自动化的人，则是 **DAC/ICCAD** 的常客，期刊投 **IEEE T-CAD**、**IEEE T-VLSI Systems**。这意味着同一个先进封装实验室里，做混合键合工艺的师兄和做多 Die 布图算法的师姐，读的论文、投的会议几乎不重叠——选方向前最好想清楚自己更想往哪一侧靠。节奏上它和器件物理那种"十年磨一界面"不同，更贴产业脉搏：一篇好的互联或布图工作，从想法到出 paper 往往一两年内见分晓。
