...
但拆开就产生了新的代价——芯片间的通信成本。信号在片内传输只需几皮秒，能耗极低；越出芯片边界、经过封装基板走线之后，延迟倍增，每比特传输能耗高出一个量级。这个代价让"分开制造"和"合并使用"之间出现了一道鸿沟。**先进封装**的核心命题，就是想尽办法把这道鸿沟填平：<u>让分离的芯片在物理上靠得更近，近到通信代价可以忽略不计</u>。
