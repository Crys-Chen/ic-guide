# 电路 - 入门速成

如果你不打算深入电路设计但又想理解硬件原理,**入门速成**板块是把"电路 + 信号系统"压缩成一学期的高质量"通识课"。这里的课程通常用更现代、面向问题的方式讲传统三门课(电路分析、模电、信号与系统)的核心内容,适合 CS 背景想转 ECE 或硬件背景想快速建立全图景的同学。

## 推荐课程

- **[UCB EE16A&B: Designing Information Devices and Systems](EE16.md)** — UC Berkeley 把电路+信号+线代+控制压成一门入门课,**国外最有名的"电路通识"课**
- **[UCB EE120: Signals and Systems](signal.md)** — Berkeley 信号与系统经典版
- **[MIT 6.007: Signals and Systems](Signals_and_Systems_AVO.md)** — MIT 信号系统入门版

## 对应的科研方向

入门速成的本质是为后续专业课打基础,**不直接对应某个研究方向**。但学完它你能:
- 看懂大部分 ECE 论文中的电路框图和信号流图
- 决定要不要深入 [模拟](../模拟/) 或 [信号处理](../信号处理/) 方向
- 与 EE 同事沟通时不被基础术语绊倒
