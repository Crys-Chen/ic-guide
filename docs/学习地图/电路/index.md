...
- **[数字逻辑基础](数字设计/数字逻辑基础/MICR130003.md)** — 逻辑门、组合/时序电路、有限状态机；CPU 与 FPGA 的最底层基础
- **[硬件描述语言](数字设计/HDL/index.md)** — Verilog（事实标准）、Chisel（参数化更强）、HLS（用 C++ 描述算法）；让你能像写代码一样"描述"硬件
- **[FPGA](数字设计/FPGA/MICR130024.md)** — 半定制可编程芯片；既是数字电路最常见的实战平台，也是研究方向
- **[数字集成电路](数字设计/数字集成电路/MICR130029.md)** — 在 CMOS 工艺层面实现数字逻辑，关心晶体管级延时/功耗/面积
- **[数字验证](数字设计/数字验证/index.md)** — testbench、断言、UVM；数字 IC 最大的就业岗位类别（征集中）
- **[ASIC 与数字后端](数字设计/ASIC与数字后端/index.md)** — 综合、布局布线、时序收敛，从 RTL 走到 GDSII

### 模拟与射频

- **[电路分析基础](模拟与射频/电路分析基础/tingyi.md)** — 欧姆/基尔霍夫定律，RLC 网络
- **[模拟电子线路](模拟与射频/模拟电子线路/MICR130002.md)** — 用晶体管搭放大器/滤波器/振荡器
- **[模拟集成电路](模拟与射频/模拟集成电路/MICR130030.md)** — 在硅片上实现高性能模拟模块（运放、比较器、PLL）
- **[ADC / DAC](模拟与射频/数模模数转换器/INFO130270.md)** — 连接模拟世界与数字世界的桥梁，是混合信号 IC 的核心
- **[射频电路](模拟与射频/射频电路/index.md)** — 从板级高频电路到片上 LNA、PA、混频器、VCO；无线通信的核心

### 信号处理

- **[信号与系统](信号处理/信号与系统/MICR130004.md)** — 傅里叶/拉普拉斯/Z 变换；通信和信号处理的数学基础
- **[数字信号处理 (DSP)](信号处理/数字信号处理/INFO130010.md)** — 数字滤波、变换、识别；从手机降噪到通信编码
...
不打算深入电路设计但又想理解硬件原理的同学，建议看[入门速成](入门速成/index.md)板块——EE16A&B 是把电路 + 信号系统压缩成一学期的高质量"通识"课；信号侧还可配合[信号与系统](信号处理/信号与系统/MIT6.003.md)目录下的 UCB EE120 和 MIT 6.007。
...
- [数字验证（SV/UVM）](数字设计/数字验证/index.md)（征集中）
- [数字后端](数字设计/ASIC与数字后端/数字后端.md)（征集中）
- [功率电子](模拟与射频/功率电子/index.md)（征集中）
- [模拟版图设计](模拟与射频/版图设计/index.md)（征集中）
- [生物电子接口电路](生物电子/index.md)（征集中）
- [自动控制原理](自动控制/index.md)（征集中）
