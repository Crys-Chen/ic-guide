# MIT 6.003: Signals and Systems

## 课程简介

- 所属大学：麻省理工学院
- 先修要求：微积分（含复数）、线性代数基础
- 编程语言：无（部分习题可选 MATLAB/Python 辅助）
- 课程难度：🌟🌟🌟🌟
- 预计学时：100 小时

MIT 6.003 是信号与系统领域最权威的教学资源之一，与 Alan V. Oppenheim 所著经典教材《Signals and Systems》直接配套。课程内容涵盖连续时间与离散时间信号的数学表示（奇异函数、复指数）、傅里叶变换（CTFT、DTFT、DFT）、拉普拉斯变换、Z 变换与采样定理，以及 LTI（线性时不变）系统的核心分析方法（冲激响应、卷积、频率响应、极零点分析）。应用方向贯穿通信系统、反馈控制与信号处理，是后续学习数字信号处理（DSP）、射频电路设计和控制理论的必备基础。

**B 站 Oppenheim 本人主讲原版录像**（MIT 课堂录像，共 26 集）是中文互联网上观看量最高的版本，可直接替代 OCW 版本：

- 英语原版（约 142 万次播放）：<https://www.bilibili.com/video/BV1CZ4y1j7hs/>
- 中英双语字幕版：<https://www.bilibili.com/video/BV1tf4y1m7Wf/>

国内高校普遍使用**郑君里版《信号与系统》**（高等教育出版社）作为教材，B 站有配套讲授（北邮版本，25 集）：<https://www.bilibili.com/video/BV1w5411h7hR/>

## 课程资源

- 课程网站（OCW，含讲义/习题/考试）：<https://ocw.mit.edu/courses/6-003-signals-and-systems-fall-2011/>
- 课程视频（B 站·英文原版）：<https://www.bilibili.com/video/BV1CZ4y1j7hs/>
- 课程视频（B 站·中英字幕）：<https://www.bilibili.com/video/BV1tf4y1m7Wf/>
- 课程教材：《Signals and Systems》Alan V. Oppenheim & Alan S. Willsky，第 2 版
- 课程作业：OCW 提供完整习题集及参考答案、历年期中/期末考试

## 进阶参考

- UCB EE120 Signals and Systems（理论更严谨，延伸至二维傅里叶与压缩感知）
- MIT 6.011 Signals, Systems and Inference（6.003 的延续，结合统计推断）：<https://ocw.mit.edu/courses/6-011-signals-systems-and-inference-spring-2018/>
