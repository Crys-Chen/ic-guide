...
- ADC/DAC 原理讲解（中文）：B 站搜索"ADC DAC 电路设计"或"数模模数转换器原理"
- SAR ADC 专题：B 站搜索"逐次逼近型 ADC"或"SAR ADC 设计"
- Sigma-Delta 调制器：B 站搜索"ΔΣ 调制器"或"过采样 ADC"
- 国内高校讲座：B 站搜索"ADC 浙大"或"数据转换器 集成电路"可找到多所高校专题课

## 前置学习建议

1. 模拟集成电路设计（运放设计、电流镜、比较器）——参见 [Razavi 模拟 CMOS 集成电路](../../模拟/模拟集成电路/razavi.md) 或同类课程
2. 数字信号处理基础（采样定理、量化噪声、频谱分析）——参见 [MIT DSP（Oppenheim）](../数字信号处理/MIT_DSP.md)
3. 运算放大器非理想特性（失调、带宽、噪声）——建议先完成模拟 IC 课程中的运放章节再进入 Pipeline/SAR 架构学习
4. 搜索 Murmann 的 ISSCC Tutorial 幻灯片作为第一份阅读材料，建立 ADC 性能空间的全局视野
