...
电路是 IC 设计研究的核心板块——几乎所有[科研方向](../../科研方向/)在某种程度上都依赖它。下面是每个子分支与具体科研方向的映射:
...
| EDA 工具 | [EDA 与设计自动化](../../科研方向/EDA与设计自动化.md)、做研究时所有"流片"环节都要用 |
| 嵌入式 SoC | [处理器架构与编译系统](../../科研方向/处理器架构与编译系统.md)、[硬件安全与可信计算](../../科研方向/硬件安全与可信计算.md) |

> 这一板块**最贴近 IC 工程师的日常**:写 RTL、跑仿真、看波形、查时序——这些技能是从课程到工业界最直接的过渡。建议至少把"数字电路 + 一门 HDL + 一个 EDA 工具"完整链条走完,即使你最终选择软件方向。
